...
#PBS -N titancam.lav-10x15-toposhortrad
...
set mpirun = /curc/tools/x_86_64/rh6/openmpi/1.6.4/intel/13.0.0/torque/4.1.4/bin/mpirun
...
set blddir       = /projects/larsonej/titancam/build_$runname
set rundir       = /lustre/janus_scratch/$LOGNAME/run/$CAM_RUN\_rh6_shortrad
...
#    echo "${0}:  copying namelist from $blddir ..." 
    cp $CAM_BASE/lists/namelist_$runname\shortrad $rundir/namelist_$runname
#  endif
...
# so be careful.
sed "s,^[ 	]*rest_pfile[ 	]*=[ 	]*'[A-Za-z0-9/_ ]*', rest_pfile		= '$rundir/cam.rpointer'," < $rundir/namelist_$runname > $rundir/tmp.namelist
mv -f $rundir/tmp.namelist $rundir/namelist_$runname

sed "s,^[ 	]*rpntpath[ 	]*=[ 	]*'[A-Za-z0-9/_ ]*', rpntpath	 	= '$rundir/lnd.rpointer'," < $rundir/namelist_$runname > $rundir/tmp.namelist
mv -f $rundir/tmp.namelist $rundir/namelist_$runname

# Set the runtype to be the value in runtype.
sed "s,^[ 	]*nsrest[ 	]*=[ 	]*[0-9], nsrest		= $runtype," < $rundir/namelist_$runname > $rundir/tmp.namelist
mv -f $rundir/tmp.namelist $rundir/namelist_$runname
...
sed -e :a -e '/,$/N; s/,\n[ ]*/, /; ta' < $rundir/namelist_$runname > $rundir/tmp.namelist
mv -f $rundir/tmp.namelist $rundir/namelist_$runname
...
#        $mpiboot -n $NNODE -f $PBS_NODEFILE -v --remcons
        $mpirun -np $nproc $rundir/cam < $rundir/namelist_$runname >&! $rundir/$outfile || echo "${0}:  CAM run failed" && exit 1
