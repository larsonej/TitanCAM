...
!
   timestep(:ncol) = L_s
   call outfld ('L_s     ',timestep, pcols, lchnk)
...
!  AJF 4-28-09 : Dry adiabatic adjustment for troposphere of jovian planet
!   Includes "nudger" to maintain lower boundary temperature near constant value

#if (! defined IDEAL_PHYS)

#ifdef _TFORCE
     flx_heat(:)=0.
     call lowbnd_adj_heat(ztodt, state, tend, ptend, flx_heat)

!!!     call adia_adj_heat(ztodt, state, tend, ptend)
     
     call physics_update(state,tend,ptend,ztodt)

!!!     call check_energy_chng(state, tend,"adia_adj_heat", nstep, ztodt,        &
!!!       zero, zero, zero, flx_heat)
       
     call check_energy_chng(state, tend,"lowbnd_adj_heat", nstep, ztodt,        &
       zero, zero, zero, flx_heat)
#endif
...
!!!   call dadadj_trop (lchnk   ,ncol    , &
!!!                   state%pmid,state%pint,state%pdel,ptend%s,ptend%q(1,1,1) )
!!!   ptend%name  = 'dadadj_trop'
!!!   ptend%ls    = .TRUE.
!!!   ptend%lq(1) = .TRUE.
!!!   ptend%s(:ncol,:)   = (ptend%s(:ncol,:)   - state%t(:ncol,:)  )/ztodt * cpair
!!!   ptend%q(:ncol,:,1) = (ptend%q(:ncol,:,1) - state%q(:ncol,:,1))/ztodt
...
!   -ajf 7-13-07
! AJF:  (1-12-09) No tidal forcing for opgcm unless _TITAN defined
! ============================================================================
!
!
#ifdef _TITAN
...
! Add forcing of zonal wind in bottom layers     

#ifdef _U_E

     call t_startf('u_bndry')
      flx_heat(:)=0.0  !not really heat
      call u_bndry ( state, tend, ptend, flx_heat )
     call t_stopf('u_bndry')

     call physics_update(state,tend,ptend,ztodt)

     call check_energy_chng(state, tend,"u_bndry", nstep, ztodt,        &
       zero, zero, zero, flx_heat)
     ...
     call t_startf('u_taux')
      call u_taux ( state, tend, ptend, taux )
     call t_stopf('u_taux')
     
#endif

#ifdef _EDDYF
     call t_startf('eddy_forcing')
      flx_heat(:)=0.
      call eddy_forcing(state, tend, ptend, flx_heat)
     call t_stopf('eddy_forcing')
     
     call physics_update(state,tend,ptend,ztodt)
       
     call check_energy_chng(state, tend,"eddy_forcing", nstep, ztodt,        &
       zero, zero, zero, flx_heat)
...
! Option to use "ideal-physics" routine tphysidl to add some idealized forcings:
#ifdef IDEAL_PHYS
     call t_startf('idlphys')
       call tphysidl (ztodt,taux,tauy, state,tend, ptend)
       call physics_update(state,tend,ptend,ztodt)
       call check_energy_chng(state, tend,"tphysidl", nstep, ztodt,        &
        zero, zero, zero, tend%flx_net)
     call t_stopf('idlphys')
#endif
...
!
      call t_startf ('radctl_opgcm')
      call radctl_opgcm (lchnk, ncol, lwup, emis, state%pmid,             &
