...
! March 23 2006, AJF:  Changed condition as to whether to include molecular 
!                      diffusion:  only run if model-top pres < 1.e-3 Pa 
!---------------------------------------------------------------------------------
...
! for each constituent is a needless expense unless the diffusivity is significant.
    if (hypm(1) .lt. 0.001) then  !!ajf, 3/23/06: changed from .1 Pa to 0.001 Pa
       do k = 1, pver
...
!!! AJF 8-21-08: Define indices for non-transported species to zero 
!!! diffusion tendency

    integer ind_ch4,ind_c2h2,ind_c2h4,ind_c2h6,ind_hcn
    integer ind_haz0,ind_haza,ind_hazb,ind_hazc
...
!!! AJF, 8-21-08:  ZERO tendencies for CH4,C2H2, C2H4, C2H6, and HCN, which
!!! are *not* meant to be transported (transported species have suffix TR on
!!! their variable names.  THESE SPECIES MUST BE INITIALIZED AS 'wet' TYPES  
!!! ALSO MUST INCLUDE HAZE_*

    call cnst_get_ind('CH4',ind_ch4)
    call cnst_get_ind('C2H2',ind_c2h2)
    call cnst_get_ind('C2H4',ind_c2h4)
    call cnst_get_ind('C2H6',ind_c2h6)
    call cnst_get_ind('HCN',ind_hcn)
    call cnst_get_ind('HAZE_0',ind_haz0)
    call cnst_get_ind('HAZE_A',ind_haza)
    call cnst_get_ind('HAZE_B',ind_hazb)
    call cnst_get_ind('HAZE_C',ind_hazc)

    do k=1, pver
       do i=1, ncol
        ptend%q(i,k,ind_ch4)=0.0
        ptend%q(i,k,ind_c2h2)=0.0
        ptend%q(i,k,ind_c2h4)=0.0
        ptend%q(i,k,ind_c2h6)=0.0
        ptend%q(i,k,ind_hcn)=0.0
        ptend%q(i,k,ind_haz0)=0.0
        ptend%q(i,k,ind_haza)=0.0
        ptend%q(i,k,ind_hazb)=0.0
        ptend%q(i,k,ind_hazc)=0.0
       enddo
    enddo 
...
!---------------------------Local workspace-----------------------------
    ...
    real(r8) :: tmp2(pcols)                        ! temporary storage
...
!!$       v(i,pver) = v(i,pver) / (1. + ztodt*dampx)
    ...
    rad = SHR_CONST_REARTH
    do i = 1, ncol
    tmp2(i) = ztodt*gravit*((rad/(rad+ztau(1)))**2)*rpdel(i,1)
    enddo

    do i = 1, ncol
       do m = 1, pcnst+pnats
          q(i,pver,m) = q(i,pver,m) + tmp1(i) * cflx(i,m)
          q(i,1,m) = q(i,1,m) !+ tmp2(i) * tflx(i,m) EJL if use friedson tracers, then uncomment this
       end do
    end do
