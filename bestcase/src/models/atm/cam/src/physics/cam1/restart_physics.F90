      ...
      if (aeres) then
         if (masterproc) then
            ...
            if (ioerr /= 0 ) then
               ...
               call endrun ('WRITE_RESTART_PHYSICS')
            end if
         endif
         ...
         if (masterproc) then
            close(nrg2)
            write(6,*) 'in atm restart physics', fname
...
!  DEBUG - PRINT OUT TS IMMEDIATELY AFTER RESTART READ       
      do i=begchunk,endchunk
        print*,' ts for chunk i = ',i
        print*,srfflx_state2d(i)%ts
      enddo
      call read_chunk_from_field(nrg,1,1,1,tmpfield)
      do i=begchunk,endchunk
        ncol = get_ncols_p(i)
      ...
      if (aeres) then
         if (masterproc) then
            ...
            if (ioerr /= 0 ) then
               ...
               call endrun ('READ_RESTART_PHYSICS')
            end if
