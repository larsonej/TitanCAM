...
!-----------------------------------------------------------------------------------------
   ...
   if ( dtime == uninit_int ) then
      if (dycore_is ('EUL')) then
         ...
         call endrun (sub//': need valid dycore to set default DTIME')
      end if
   end if
...
! Local variables
   ...
   type(esmf_date) :: temp_date   ! temporary date used in logic
...
! Initialize stop date.
   ...
   stop_date = esmf_dateinit(cal_type, 99991231, stop_tod, rc)
   run_length_specified = .false.

   if ( stop_ymd /= uninit_int ) then
      temp_date = esmf_dateinit(cal_type, stop_ymd, stop_tod, rc)
      call chkrc(rc, sub//': error processing stop_ymd')
      call esmf_dateislater(temp_date, stop_date, islater, rc)
      call chkrc(rc, sub//': error from esmf_dateislater while processing stop_ymd')
      if ( islater ) then
         stop_date = temp_date
      end if
   ...
   if ( nestep /= uninit_int ) then
      if ( nestep >= 0 ) then
         temp_date = esmf_dateincrementsec(start_date, dtime*nestep, rc)
      else
         temp_date = esmf_dateincrementday(start_date, -nestep, rc)
      end if
      call chkrc(rc, sub//': error processing nestep')
      call esmf_dateislater(temp_date, stop_date, islater, rc)
      call chkrc(rc, sub//': error from esmf_dateislater while processing nestep')
      if ( islater ) then
         stop_date = temp_date
      end if
      run_length_specified = .true.
   ...
   if ( nelapse /= uninit_int ) then
      if ( nelapse >= 0 ) then
         temp_date = esmf_dateincrementsec(start_date, dtime*nelapse, rc)
      else
         temp_date = esmf_dateincrementday(start_date, -nelapse, rc)
      end if
      call chkrc(rc, sub//': error processing nelapse')
      call esmf_dateislater(temp_date, stop_date, islater, rc)
      call chkrc(rc, sub//': error from esmf_dateislater while processing nelapse')
      if ( islater ) then
         stop_date = temp_date
      end if
      run_length_specified = .true.
...
! Initialize stop date.
   ...
   if ( stop_ymd /= uninit_int ) then
      temp_date = esmf_dateinit(rst_type, stop_ymd, stop_tod, rc)
      call chkrc(rc, sub//': error processing stop_ymd')
      call esmf_dateislater(temp_date, stop_date, islater, rc)
      call chkrc(rc, sub//': error from esmf_dateislater while processing stop_ymd')
      if ( islater ) then
   ...
   if ( nelapse /= uninit_int ) then
      if ( nelapse >= 0 ) then
         temp_date = esmf_dateincrementsec(curr_date, dtime*nelapse, rc)
      else
         temp_date = esmf_dateincrementday(curr_date, -nelapse, rc)
      end if
      call chkrc(rc, sub//': error processing nelapse')
      call esmf_dateislater(temp_date, stop_date, islater, rc)
      call chkrc(rc, sub//': error from esmf_dateislater while processing nelapse')
...
subroutine get_curr_titan_calday(frac_day, day_in_year, offset)
  implicit none
   ...
   day_earth = day_earth + 365*(yr - y0) + leap_days
   
   frac_day = day_earth/PLANET_DAY_RATIO - FLOOR(day_earth / PLANET_DAY_RATIO)
   day_in_year = day_earth - (365*PLANET_YEAR_RATIO)*FLOOR(day_earth/(365*PLANET_YEAR_RATIO))
! EJL 11/12-Changed TITAN to PLANET in above to generalize for opgcm
end subroutine get_curr_titan_calday
...
!-----------------------------------------------------------------------------------------
   ...
   date1 = esmf_dateinit(cal_type, ymd1, tod1, rc)
   call chkrc(rc, sub//': error return from esmf_dateinit: setting date1')

   date2 = esmf_dateinit(cal_type, ymd2, tod2, rc)
   call chkrc(rc, sub//': error return from esmf_dateinit: setting date2')

   diff = esmf_timeinit()
   call esmf_datediff(date1, date2, diff, islater, rc)
   call chkrc(rc, sub//': error return from esmf_datediff')
   call esmf_timeget(diff, ndays, nsecs, rc)
   call chkrc(rc, sub//': error return from esmf_timeget')
   days = ndays + nsecs/86400._r8
   if (.not. islater) days = -days
