...
!   interface with cpl6.
!
! CVS Id: $Id: ccsm_msg.F90 62 2008-04-23 22:59:18Z cam_titan $
! 
!-----------------------------------------------------------------------
...
!--------------------------------------------------------------------------
! Private data local to this module only
!--------------------------------------------------------------------------
...
!
  ...
  real(r8), private             :: rbuff(cpl_fields_rbuf_total) ! Floating pt buffer to/from cpl
...
!
!===============================================================================
! The following first set of subroutines can be publically called.
! After this set are the subroutines that are callable only from within
! this module.
!===============================================================================
!
  subroutine ccsmini
...
!
       if (masterproc) then
          write(6,*) '(CCSMINI): is_first_step is true; get albedos from coupler'
          call shr_sys_flush(6)
       end if
...
!
    if ( (.not. csmstop) .and. (ibuff(cpl_fields_ibuf_stopeod)/=0) ) then
       csmstop = .true.
       if (masterproc) write(6,*) &
            '(CCSMRCV): received stop at end of day signal from flux coupler'
    end if
!
! Determine if write restart at end of day
!
    if ( (.not. csmrstrt) .and. (ibuff(cpl_fields_ibuf_resteod)/=0) ) then
       csmrstrt = .true.
       if (masterproc) write(6,*) &
       '(CCSMRCV): received write restart at end of day signal from flux coupler'
...
!---------------------------Local workspace-----------------------------
    ...
    integer cdatecsm,cseccsm         ! date,sec at beginning of current timestep
...
!
   if (masterproc) then
      ...
      if (ioerr /= 0 ) then
         ...
         call endrun ('WRITE_RESTART_CCSM')
      end if
   endif
!
...
!
   if (flxave) then
      ...
      if (.not. allocated(recv2d_chunk))then
         allocate(recv2d_chunk(pcols,nrcv,begchunk:endchunk), STAT=ierr)
         if (ierr /= 0) then
            call endrun ('(READ_RESTART_CCSM) recv2d_chunk allocation error')
         endif
      end if
...
!-----------------------------------------------------------------------
     ...
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) rho allocation error')
     endif
     allocate (netsw   (pcols,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) netsw allocation error')
     endif
     allocate (psl     (pcols,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) psl allocation error')
     endif
     allocate (precca  (pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) precca allocation error')
     endif
     allocate (precla  (pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) precla allocation error')
     endif
     allocate (precsca (pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) precsca allocation error')
     endif
     allocate (precsla (pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) precsla allocation error')
     endif
     allocate (rainconv(pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) rainconv allocation error')
     endif
     allocate (rainlrsc(pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) rainlrsc allocation error')
     endif
     allocate (snowconv(pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) snowconv allocation error')
     endif
     allocate (snowlrsc(pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) snowlrsc allocation error')
     endif
     allocate (prc_err (pcols ,begchunk:endchunk), STAT=ierr)
     if (ierr /= 0) then
        call endrun ('(INITIALIZE_CCSM_MSG) prc_err allocation error')
     endif
!
...
# if (defined FV_BLOCK)
       ibuff(cpl_fields_ibuf_lisize) = endlonxy-beglonxy+1  ! number of local longitudes
       ibuff(cpl_fields_ibuf_ljsize) = endlatxy-beglatxy+1  ! number of local latitudes
# else
...
!-----------------------------------------------------------------------
    write(6,*)'(CCSM_MSG_COMPAT): This is revision: $Revision: 1.1 $'
    write(6,*)'              Tag: $Name:  $'
