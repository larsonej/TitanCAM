...
! !USES:
      ...
      use commap
      use prognostics
      use physconst,    only: gravit,pstd
      use history,      only: fillvalue
      use constituents, only: pcnst, pnats, cnst_name, qmin, cnst_read_iv
      use carma,	only:carma_implements_cnst, carma_init_cnst
      use chemistry,    only: chem_implements_cnst, chem_init_cnst
      ! TBH:  combine modules aerosol_intr and aerosols?  EJL 7-7-13 commented
      ! out aerosol routines we do not use  
...
!BOC
!
!      integer,	intent(in), optional	:: m_cnst	! carma constituent index
!
! !LOCAL VARIABLES:
...
! FAO:  ensure that constituents are properly initialized from file
!            if (cnst_read_iv(m) .and. &
!                 nf_inq_varid(ncid_ini, cnst_name(m), tracid(m) ) /= NF_NOERR ) then
!               call endrun ('READ_INIDAT'//cnst_name(m)//' not found on IC file')
!            end if
...
! Search among constituents
            ...
            enddo
            if (mq .eq. 0) then
               call endrun ('READ_INIDAT:Q not found - exiting')
            endif
            m = mq
...
! Initialize instead of input
               ...
               arrxzy = 0.
!			   print*,carma_implements_cnst(cnst_name(m))
...
!                  write(6,*) '          ', cnst_name(m), ' initialized by "chem_init_cnst"'
                  ...
                  call tracers_init_cnst(cnst_name(m), arrxzy)
!                  write(6,*) '          ', cnst_name(m), ' initialized by "tracers_init_cnst"'
	           else if (carma_implements_cnst(cnst_name(m))) then
                  call carma_init_cnst(cnst_name(m), arrxzy)
!                  write(6,*) '          ', cnst_name(m), ' initialized by "carma_init_cnst"'
               else
...
! Search among constituents
            ...
            enddo
            if (ml1 .eq. 0) then
               call endrun ('READ_INIDAT:CLDLIQ not found - exiting')
            endif
            if (ml2 .eq. 0) then
               call endrun ('READ_INIDAT:CLDICE not found - exiting')
            endif
            m = ml1
...
!
! SJL:
!!!!!         tmass0 = 98222./gravit  !for Earth

         tmass0 = pstd/gravit  ! ajf, 2-16-07, (ps0 not available)
!
...
!!!!         if (ideal_phys) tmass0 = 100000./gravit
         if (ideal_phys) tmass0 = pstd/gravit
      ...
      do m = 1, pcnst+pnats
         ...
         if (masterproc) then
            ...
            if (cnst_read_iv(m) .and. &
               ...
               arrxzy = 0.
print*, m, cnst_name(m)
print*,'cldcond_implements_cnst', cldcond_implements_cnst(cnst_name(m))
print*,'chem',chem_implements_cnst(cnst_name(m))
print*,'tracers',tracers_implements_cnst(cnst_name(m))
!print*,'aerosol',aerosol_implements_cnst(cnst_name(m))
print*,'carma',carma_implements_cnst(cnst_name(m))
               if (cldcond_implements_cnst(cnst_name(m))) then
                  call cldcond_init_cnst(cnst_name(m), arrxzy)
                  write(6,*) '          ', cnst_name(m), ' initialized by "cldcond_init_cnst"'
print*,'test1'
