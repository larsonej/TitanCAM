...
c
c   These scale factors (the amount of actual measurable cartesion distance
...
c
c
c  Define appropriate sign factor for <w>.  Non-vertical-cartesion coordinates
...
c
     ...
     $    t(ix,iy,k),rmu(k),thcond(k),dkz(ix,iy,k)
      enddo
c
c
...
c
c
c  Write pressure and diffusion coefficient values for graphing
c
c     do k = 1,NZ
c       write(LUNOTEMP,10) p(1,1,k)
c       write(LUNOTEMP,11) dkz(1,1,k)
c     enddo
c
c
...
c
c      <coeff>, <acoeff>, are nitrogen coefficients for thermal
c         conductivity eqn (for thermal conductivity in W/(m*K) and
c         density in g/ml)

       dimension coeff(4), acoeff(0:4)
c
c
c  Initialization of input data for viscosity and thermal conductivity
c  equations.  Data from Thermophysical Properties of Fluids
c  [J. Phys. Chem. Ref. Data. 16, 993 (1987)]

       data coeff/ 3.3373542, 0.37098251, 0.89913456, 0.16972505/
       data acoeff/ 0.46649, -0.57015, 0.19164, -0.03708, 0.00241/
c
c
c   Define formats
c
    3 format(e6.2, e13.4)  ! input T and rho from TAEM file
    4 format(f10.6,5x,f7.3,5x,1pe11.5)  ! input from HASI data file (z, T, rhoa)
c
c
c  Announce entry to this routine
c
...
c
      if( NZ.eq.50 ) then
        dzfix = 2.e5
        write(*,*) '100 km model with 2 km spacing'
      elseif( NZ.eq.60 ) then
        dzfix = 10.e5
        write(*,*) '600 km model with 10 km spacing'
      else
        write(*,*) '<initatm> Invalid NZ ',NZ
        stop
      endif
c
c
...
c  Temperature and air density determined from HASI data
         read (LUNITRHO1,4) z, tair, rhoair
         z = z * 1.d5 ! convert km -> cm

c    Interpolate to match CARMA grid
         do while( z .lt. zc(ix,iy,k) )
          zlow = z
          tlow = tair
          rholow = rhoair
          read (LUNITRHO1,4) z, tair, rhoair
         ...
         t(ix,iy,k) = (tair - tlow)/(z - zlow) *(zc(ix,iy,k) - z) + tlow
         rhoa(ix,iy,k) = (rhoair - rholow)/(z - zlow) *(zc(ix,iy,k) - z)
     $                           + rholow
        
        else
c
c  Temperature and air density determined from Lellouch and Hunten
c  recommended model (Titan Atmosphere Engineering Model 1989)
c
         read (LUNITRHO1,3) t(ix,iy,k), rhoa(ix,iy,k)
...
c
c
c  Calculate pressure from gas law (equation of state)
c 
        p(ix,iy,k) = rhoa(ix,iy,k) * R_AIR * t(ix,iy,k)

       !print *,t(ix,iy,k),rhoa(ix,iy,k),p(ix,iy,k)
...
c
        ...
        if( do_huygens_Tprofile ) then 
           ...
           t(ix,iy,k) = (tair - tlow)/(z - zlow) *(zc(ix,iy,k) - z) 
     $                         + tlow
           rhoa(ix,iy,k) = (rhoair - rholow)/(z - zlow) * 
     $                        (zc(ix,iy,k) - z) + rholow
...
c       print *,t(ix,iy,k),rhoa(ix,iy,k),p(ix,iy,k),
c    $      p(ix,iy,k-1) *exp( -dzfix*GRAV/(R_AIR*tbot) )

          tbot = 0.5 * ( t(ix,iy,k-1) + t(ix,iy,k) )
c         p(ix,iy,k) = p(ix,iy,k-1) *
c    $                 exp( -dzfix*GRAV/(R_AIR*tbot) )
          ptc(ix,iy,k) = rhoa(ix,iy,k) * 
     $                   t(ix,iy,k) * ( PREF/p(ix,iy,k) )**RKAPPA
...
c      Air viscosity from Lorenz [Planet. Space Sci. 41(9). 647-655 (1993)]
c
        rmu(k) = 1.718d-5 + 5.1d-8 * (t(1,1,k) - 273.)
c
c
c      Thermal conductivity of dry air from Thermophysical Properties of Fluids
c      [J. Phys. Chem. Ref. Data. 16, 993  (1987)]
c      Valid for T = triple point - 3000K, rho < 1.09 g/cm3
c
c       Viscosity term
        sigma = 0.36502496d-9  ! [m]
        epsilonk = 100.01654   ! epsilon/k_B [K]
        e1 = 5./16. * 1/sigma**2 *    ! eta0 w/o omega term in mks
     $           sqrt(WTMOL_AIR * BK /PI /AVG /1.d5)
        omega = 0.  !ln(omega)
        do i = 0,4
           omega = omega + acoeff(i)*(dlog( t(1,1,k) / epsilonk ))**i
        enddo
        eta0 = e1 * sqrt(t(1,1,k)) / exp(omega)  ![mks]

c       Dilute gas function thermal conductivity term
        rhoc    = 314.  !critical density of N2 [kg/m3]
        thcondc = 4.17  !critical thermal conductivity
        x1 = 0.95185202
        x2 = 1.0205422

        Cv = (CP - R_AIR)/1.d4  ![mks]
        c1 = 2.5*(1.5 - x1 )  ! lambda_tr w/o F

        bk_mks = BK / 1.d7
        thcond0 = bk_mks*AVG * eta0    ! lambda_0
     $             /(WTMOL_AIR/1.d3) * ( c1 + x2*(Cv/bk_mks/AVG + x1))

c       Excess function thermal conductivity term
        rhor = rhoa(1,1,k)/rhoc * 1.d3  
        tsum = 0.
        do i=1,4
                tsum = tsum + coeff(i) * rhor**i
        enddo
        thcondR = thcondc * tsum

        thcond(k) = thcond0 + thcondR

c       Convert mW/m/K to cgs (erg/s/cm/K)
        thcond(k) = 1.0d2 * thcond(k)

      enddo
c
c
c  Define vertical wind speed & diffusion coefficients [in metric cgs units]
c  from Toon et al. (1992) revised model
c
      do k=1,NZP1
        do ix=1,NX
          do iy=1,NY
...
c
     ...
     $              ( t(1,1,k) / rmu_t0 )**1.5d0
        thcond(k) = ( 5.69 + .017*( t(1,1,k) - T0(1) ) )*4.18e2
      enddo
c
c
c  Define vertical wind speed & diffusion coefficients [in metric cgs units]
...
c
     ...
     $              1, xc3(ixyz),yu3(ixyz), rlon_yu,rlat_yu)
        ...
        ds_x = RTITAN * sfirdis(rlon_xl,rlat_xl, rlon_xu,rlat_xu)
        ds_y = RTITAN * sfirdis(rlon_yl,rlat_yl, rlon_yu,rlat_yu)
...
c
      ...
      if( pl2(1, NZP1) .lt. 8.e5 .or. pl2(1, NZP1) .gt. 1.2e6 ) then
...
c
c
c  Compute constants used in viscosity ! EJL - maybe should be changed to values in Wikipedia
c
      rmu_0 = 1.8325e-4
      rmu_t0 = 296.16
      rmu_c = 120.
