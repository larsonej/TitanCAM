...
!cdir nodep
    do p = begp,endp
       rbufslp(p) = (t10(p) - (SHR_CONST_TKFRZ - 5.0))*dtime / SHR_CONST_CDAY
       if (rbufslp(p) < 0._r8) rbufslp(p) = -99999.
    end do
...
!cdir nodep
    do p = begp,endp
       itypveg = itype(p)
       rbufslp(p) = max(0.0, (t10(p) - (SHR_CONST_TKFRZ+pftpar(itypveg,31))) &
...
!cdir nodep
    do p = begp,endp
       rbufslp(p) = max(0.0, (t_ref2m(p) - (SHR_CONST_TKFRZ + 5.0)) &
            * dtime/SHR_CONST_CDAY)
    end do
