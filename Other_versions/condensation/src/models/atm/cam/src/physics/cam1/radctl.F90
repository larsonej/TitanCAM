...
subroutine set_aerosol_mmr(mmr)
!----------------------------------------------------------------------- 
! This subroutine is a kludge, designed to artificially set the 
! aerosol mixing ratios to reporoduce observed optical depths
!----------------------------------------------------------------------- 
  use shr_kind_mod, only: r8 => shr_kind_r8
  use ppgrid
  use abortutils, only: endrun
  use radcnst, only: Na, Nv_p1, mmraero

  implicit none

  real(r8), intent(out) :: mmr(pcols,pver,Na) ! aerosol mass mixing ratio
!
!---------------------------Local variables-----------------------------
!
  integer i, a, k

  if (pver /= Nv_p1 - 1) then
     call endrun ('FAO[endrun]:  invalid number of vertical levels')
  endif

  do i=1,pcols
     do a=1,Na
     do k=1,pver
        mmr(i,k,a) = mmraero(k,a)
     enddo
     enddo
  enddo

end subroutine set_aerosol_mmr
...
! FAO
   ...
   use radcnst, only: Nf, Ng,Na,ztau, Nf_lw, Ng_lw, wn_lw
...
!EJL
   use carma, only: carma_get_mmr, ncarma_bins
...
!  real(r8) fsds(pcols)          ! Flux Shortwave Downwelling Surface
   ...
   real(r8) :: aertau(pcols,Nf,ncarma_bins) ! Aerosol column optical depth - EJL changed for Titan
...
!  FAO
   ...
   real(r8) rlons(200), rlats(200)
   real(r8) mmr_g(pcols,pver,Ng) ! gass mass mixing ratio
   real(r8) mmr_a(pcols,pver,ncarma_bins) ! aerosol mass mixing ratio - EJL
   real(r8) Ti
   integer Zg, Za, Zd, Zf
...
! EJL
!   real(r8) :: mmr_carma(pcols,pver)
   real(r8) :: carmamix(pcols,pver,ncarma_bins)
...
!
      ...
      ! FAO:  kludge for mass mixing ratios
!      call set_aerosol_mmr(mmr_a)

!      open(unit=99,file='jpla_mmr.txt',status='unknown')	 
!	 write(99,*) mmr_a(1,:,1)
!     close(unit=99)

	  ! EJL - call carma instead - this needs to be fixed
	  ! Currently, just setting all aerosols to mass mixing ratio of 
	  ! the summed carma aerosols. This is wrong. I need to read in 
	  ! the aerosol optical constants and use the bins.
      call carma_get_mmr(state,carmamix)
...
!      open(unit=99, file='carma_mmr.txt',status='unknown')	  
!	 write(99,*) mmr_a(1,:,1) 
!     close(unit=99)
      ...
      do i=1,pcols
         do Zg=1,Ng
            mmr_g(i,k,Zg) = 0
         enddo
         mmr_g(i,k,1) = state%q(i,k,48) ! EJL - Changed from 4 to 48. I think this was supposed to use
					! the advected methane, but I changed it to the nonadvected.
      enddo
      enddo
      
!	  open(unit=99,file='methane.txt', status='unknown')
!	  do i=1, ppcnst
!	  write(99,*) cnst_name(i), state%q(:,:,i)
!	  enddo
!	  close(unit=99)
      ...
      if (radforce) then
         ...
         call radcswmx(lchnk   ,ncol ,pnm     ,pbr     , t, &
                    mmr_g   ,mmr_a   ,cld     ,cicewp  ,cliqwp  ,rel     , &
...
!
      ...
      call get_rlon_all_p(lchnk, 200, rlons)
      call get_rlat_all_p(lchnk, 200, rlats)
...
#ifdef DIUR_AV
         tmp_qrs(:ncol,:pver)=qrs(:ncol,:pver)
#endif        
...
!
      ...
      ! AJF:  If moved, then pressures must bee defined in destination routine
      do i=1,ncol
         do k=2,pver
         ...
         tref_u=tref(i)
         ts_u  =ts(i)
!!!         p1=pmid(i,1)  !Linear extrapolation of T to top interface:
!!!         p2=pmid(i,2)
!!!         pt=pint(i,1)
!!!         tt=(p2-pt)/(p2-p1)*t(i,1)+(pt-p1)/(p2-p1)*t(i,2)
...
!
      if (trace_gas) then
         ...
         call radclwmx_titan(lchnk, ncol, qm1, pint, t, planckf, planckfg, &
              qrl,flns,flnt,flut,flnsc,flntc,flutc,flwds,fcnl,fnl)
              
         call radclwmx_titan(lchnk,ncol, mmr_g(:,:,1), pint, planckf, planckfg, &
              qrl,flns,flnt,flut,flnsc,flntc,flutc,flwds,fcnl,fnl)
