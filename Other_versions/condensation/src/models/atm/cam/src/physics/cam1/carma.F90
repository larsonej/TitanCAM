...
! Public Data
  real(r8) ,public               :: kcarma(NELEM, NBIN, nspint)      ! carma constituent mass extinction coefficient (m2 g-1)
...
! Private data
  ...
  character(len=256)                  :: carma_optics_file = 'carma_optics_file.nc'  ! name of optics file
...
!-----------------------------------------------------------------------
  ...
  ! Have CARMA initialize its element and gas tables.
   call setupaer   ! EJL 2-13-13 Changed to setupaer to match what Erika did
!  call defineaer 
...
!write(6,*) 'ending initatm'
    ...
    ! Set names of constituent sources and declare them as history variables.  EJL - 4-11-13
...
!EJL 4-11-13 rearranged above and added if statement
...
!EJL 2-13-13 Doesn't like T0 after change
          thcond(:) = (5.69 + .017 * (t3(:) - T0(1))) * 4.18e2
          rmu(:) = (1.781e-4 * (300.55 + 111.0)) / (t3(:) + 111.0) * (t3(:) / 300.55)**1.5d0 
!	  thcond(:) = 
...
!EJL 4-11-13          call outfld(trim(cnst_name(mm))//'SF', cflx(:, mm), pcols, lchnk)
		
        ! Only put dust into the smallest bin. THIS IS 50 NM monomer from toa case
        if (cnst_name(mm) == "DUST11") then  !!11 = 42nm monomers, 20 = .67 um fractals
          ptend%lq(mm) = .true.
...
!                  massflux = (rate * rmass(j, igelem(i)) * 1.0e-3_r8 * 1.0e6_r8)							
                  ...
                  carma_emis_expected =3.e-13             ! total emission [kg/m^2/s]
...
!    close(unit=99)
    ...
    do i = 1, 1 !NELEM EJL 4-12-13 only output DUSTDD
      do j = 1, NBIN
        m = (i-1)*NBIN + j
        mm = icnst4elem(m)
...
!           kg/kg/s = kg/kg * m/s / m
!            timestep=1.8e3
            ptend%q(icol, pver, mm) = state%q(icol,pver,mm)*(vf(pver+1,j,i)*1.0e-2 / (state%zi(icol, pver) - state%zi(icol, pver+1))) 
...
#if ( defined SPMD )
    call mpibcast(kcarma, NELEM * NBIN * nspint, mpir8, 0, mpicom)
    call mpibcast(wcarma, NELEM * NBIN * nspint, mpir8, 0, mpicom)
    call mpibcast(gcarma, NELEM * NBIN * nspint, mpir8, 0, mpicom)
#endif


!open(unit=99, file='optics.txt', status='unknown')
!write(99,*) wcarma
!write(99,*) ''
!write(99,*) gcarma
!close(unit=99)
