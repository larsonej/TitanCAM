...
! March 23 2006; AJF:  Changed "0.61" to zvir throughout module
!---------------------------------------------------------------------------------
...
! PBL limits
!
!!! AJF 9-26-08, change ustar_min from 0.01 m/s to 0.0005
  real(r8), parameter :: ustar_min = 0.0005      ! min permitted value of ustar
  real(r8), parameter :: pblmaxp   = 4.e4        ! pbl max depth in pressure units
!!! AJF 9-26-08, change zkmin from 0.01 m2/s to 0.001
  real(r8), parameter :: zkmin     = 0.001        ! Minimum kneutral*f(ri)
!
! PBL Parameters
!
...
!-----------------------------------------------------------------------
   ...
   !
    ...
    real(r8), intent(out) :: kvq(pcols,pverp)         ! diffusivity for constituents
   ...
   !
    ...
    real(r8) :: kvf_high(pcols,pverp)   ! Additional high-alt diffusivity [ajf]
   ...
   !
!!!!!!! ajf, 1-23/29-08: added pmid, kvf_high to argument list
    call austausch_atm(lchnk,ncol,ri,s2,pmid,kvf,kvf_high) 

!EHW added kvq to arg list, set kvq = kvf     08/28/08
    kvq(:ncol,:) = kvf(:ncol,:)
    
   ! 
   ! Get pbl exchange coefficients
   !
   ! ajf 1/29/08 - added kvf_high to arg list
    call austausch_pbl(lchnk   ,ncol    ,                            &
                       z       ,kvf, kvf_high ,kqfs  ,khfs    ,kbfs  , &
...
!-----------------------------------------------------------------------
   ...
   !
    do i=1,ncol
       kbfs(i)      = khfs(i) + zvir*th(i,pver)*kqfs(i,1)
       thvsrf(i)    = th(i,pver)*(1.0 + zvir*q(i,pver,1))
   ...
   !
    call virtem(ncol, pcols, pver, th      ,q(1,1,1),zvir ,thv)
...
!-----------------------------------------------------------------------
   ...
   !
    do i=1,ncol
       check(i)     = .true.
       rino(i,pver) = 0.0
       thvref(i)    = th(i,pver)*(1.0 + zvir*q(i,pver,1))
       pblh(i)      = z(i,pver)
    end do
   !
   !
   ...
   !
!!!! RESET MIN PBLH TO 50 m, ajf 3/23/06
    do i=1,ncol
       if (check(i)) pblh(i) = z(i,pverp-npbl)
!!!!       pblh(i) = max(pblh(i),700.0*ustar(i))
        pblh(i) = max(pblh(i),50.0)  !min set to 50 m, arbitrary!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!===============================================================================
  subroutine austausch_atm(lchnk,ncol,ri,s2,pmid,kvf,kvf_high)
!----------------------------------------------------------------------- 
! 
! Purpose: 
...
! Author: B. Stevens (rewrite, August 2000)
! 
! AJF 1-29-08 : Add option for additional diffusivity, kvf_high
!               applied differently to kvm, kvh, in austausch_pbl
...
!-----------------------------------------------------------------------
   ...
   !
    ...
    real(r8), intent(out) :: kvf_high(pcols,pverp)  ! Additional high-alt diff, ajf 1-29-08
   !
   !---------------------------Local workspace-----------------------------
   !
    real(r8) :: fofri                  ! f(ri)
    real(r8) :: kvn                    ! neutral Kv
    real(r8), parameter :: kvf0 = 8.0e-3  !pre-exponential for ad-hoc diffusion
    real(r8), parameter :: pk_high = 1.e4 ! (100 mb) Pascals, cutoff for kvf_high
   ...
   !
    ...
    kvf_high(:ncol,1:ntop_turb) = 0.0
!!!    do k=1,ntop_turb
!!!      do i=1,ncol
!!!       kvf(i,k) = kvf0*(pmid(i,pver)/pmid(i,ntop_turb)) !ajf
!!!      enddo
!!!    enddo
   !
   ! Compute the free atmosphere vertical diffusion coefficients: kvh = kvq = kvm. 
   !
!!! ajf, 1-29-08: Add eddy diffusion, significant only near top,
!!! to stabilize inertial instability
    ...
    do k = ntop_turb, nbot_turb-1
       do i=1,ncol
          ...
          if (pmid(i,k+1) <= pk_high) then 
             kvf_high(i,k+1)= kvf0*(pmid(i,pver)/pmid(i,k+1)) !ajf - difference between pmid and pint here insignificant
          else                  !Make sure kvf_high does not interfere with PBL
             kvf_high(i,k+1)=0.0
          endif
       end do
    end do
...
!  ajf, 1-29-08: add kvf_high for smoothing momentum dist at high altitudes
    real(r8), intent(in) :: kvf_high(pcols,pverp)   ! Additional high-alt diff
    ...
    real(r8), intent(in) :: obklen(pcols)           ! Obukhov length
    real(r8), intent(in) :: ustar(pcols)            ! surface friction velocity [m/s]
    real(r8), intent(in) :: wstar(pcols)            ! convective velocity scale v[m/s]
!
! Output arguments
!
    real(r8), intent(out) :: kvm(pcols,pverp)       ! eddy diffusivity for momentum [m2/s]
...
!
    do k=1,pverp
       do i=1,ncol
          kvm(i,k) = kvf(i,k)+kvf_high(i,k)
          kvh(i,k) = kvf(i,k)+kvf_high(i,k)
          cgh(i,k) = 0.0
          cgs(i,k) = 0.0
       end do
    end do
