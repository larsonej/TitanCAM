...
!  Subroutine GlobalWn(Arr, Ck_loc)
...
!  EJL - commented out based on AJF's changes
...
!##############################################################################
  ...
  Subroutine Sort(Arr,N)
    ...
    if (masterproc) print*,'Sort: iam, size(Arr)=',iam, size(Arr)
...
!    integer,  allocatable, intent(out)    :: nsa(:)
    integer                :: nsa(:)
    real(r8), intent(in)                  :: comb_wn_grd(:)
    real(r8), intent(in)                  :: w(:)
!    real(r8), allocatable, intent(out)    :: wts(:,:)
	real(r8)                :: wts(:,:)
    integer                               :: i,j
    integer                               :: Nwn        
...
!    allocate(wts(nsm,Nwn))
  ...
  subroutine union (arr1, arr2, union_arr)
    real(r8), intent(in)                  :: arr1(:), arr2(:)
!    real(r8), allocatable, intent(out)    :: union_arr(:)
    real(r8) :: union_arr(:)
    integer                           :: i, counter
