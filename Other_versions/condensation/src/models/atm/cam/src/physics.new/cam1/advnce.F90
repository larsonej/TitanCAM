...
!-----------------------------------------------------------------------
  ...
  use carma, only: carma_timestep_init
...
!
  ...
  dochem = nstep.eq.0 .or. ichem.eq.1  .or. (mod(nstep-1,ichem).eq.0 .and. nstep.ne.1)
...
!
! Aerosol microphysics time dependent initialization, if appropriate
