<!DOCTYPE CW><CW>
<customwidgets>
    <customwidget>
        <class>FieldListBox</class>
        <header location="local">fieldlistbox.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
        <signal>selected(int)</signal>
        <slot access="protected">ClearAll()</slot>
        <slot access="protected">Reset()</slot>
        <slot access="protected">SetAll()</slot>
        <slot access="public">keyPressEvent(QKeyEvent* keyEvent)</slot>
        <slot access="public">focusOutEvent(QFocusEvent*)</slot>
    </customwidget>
    <customwidget>
        <class>DatasetSelector</class>
        <header location="global">datasetselector.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
    </customwidget>
    <customwidget>
        <class>FieldPlotWidget</class>
        <header location="global">fieldplotwidget.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
    </customwidget>
    <customwidget>
        <class>Plot</class>
        <header location="global">plot.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
        <slot access="public">AutoScale(Axis)</slot>
    </customwidget>
    <customwidget>
        <class>Map</class>
        <header location="global">globalmap.h</header>
        <sizehint>
            <width>-1</width>
            <height>-1</height>
        </sizehint>
        <container>0</container>
        <sizepolicy>
            <hordata>5</hordata>
            <verdata>5</verdata>
        </sizepolicy>
        <pixmap>
            <data format="XPM.GZ" length="646"></data>
        </pixmap>
    </customwidget>
</customwidgets>
</CW>
