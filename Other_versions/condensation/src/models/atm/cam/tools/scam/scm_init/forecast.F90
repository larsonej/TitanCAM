...
!
   ...
   real(r8), intent(inout) :: q3(plev,pcnst+pnats)   ! constituent conc(tim
   real(r8), intent(inout) :: q3m1(plev,pcnst+pnats)   ! constituent conc(tim
   real(r8), intent(inout) :: q3m2(plev,pcnst+pnats)   ! constituent conc(time n: h2o first)
   real(r8), intent(in) :: etamid(plev)       ! vertical coords at midpoints
   real(r8), intent(inout) :: qfcst(plond,plev,pcnst)  
...
!!$   real(r8) qdiff(plev,pcnst)
   ...
   real(r8) d_qdvt
   real(r8) d_dqfxt
   real(r8) dqv(plev,pcnst+pnats)   ! constituent diffusion tendency
   save dqv
   real(r8) qphys(plev,pcnst+pnats)   ! constituent diffusion tendency

   real(r8) dqfx3m1(plev,pcnst) ! q tendency due to mass adjustment
!
   real(r8) qmassb(pcnst+pnats)     ! constituent mass integral before advection
...
!
   ...
   real(r8) qvadv(plev,pcnst+pnats)!estimate of vertical advection on q
   real(r8) qvadv1(plev,pcnst+pnats)!estimate of vertical advection on q
!
...
!
   ...
   call plevs0(nlon    ,plond   ,plev    ,psm2   ,pintm2  ,pmidm2 ,pdelm2)
!
...
!            qfcst(1,k,m) = qminus(1,k,m) +  divq3d(k,m)*ztodt
      ...
      do m=1,pcnst
         do k=1,plev
            qfcst(1,k,m) = qminus(1,k,m) +  divq3d(k,m)*ztodt
!            write(6,'(a,i,a,i,a,i,a,z16,ES30.16)'),'qfcst(',i,',',k,',',m,')=',qfcst(i,k,m),qfcst(i,k,m),'qminus(',i,',',k,',',m,')=',&
...
!
   ...
   call plevs0(nlon    ,plond   ,plev    ,psm1   ,pintm1f  ,pmidm1f ,pdelm1f)
...
!
!
      write (6,1105) qmassb(1), qmassf, ptb, ptf
...
!!$   call outfld('TVADV'   ,tvadv,plond,dummy)
!!$   call outfld('QVADV'   ,qvadv,plond,dummy)
!
...
!!$         end do
!!$!
!!$         call outfld('TRELAX',relaxt,plond,dummy )
!!$         call outfld('QRELAX',relaxq,plond,dummy )
!!$         call outfld('TAURELAX',rtau,plond,dummy )
!!$      end if
!!$   end if
!     
...
!
   do k = 1, plev
      ...
      vdiff(k) = v3(k)   - vobs(k)
!
! fix divt3d/divq3d to get bit for bit 
! just a place holder if needed
      if (fix_div3dfrc) then
      endif
   end do
...
!
   call outfld('TOBS',tobs,plond,dummy)
   call outfld('QOBS',qobs,plond,dummy)
   call outfld('TDIFF',tdiff,plond,dummy)
   call outfld('QDIFF',qdiff,plond,dummy)
   if( use_iop ) then
      call outfld('DIVQ',divq,plond,dummy)
      call outfld('DIVT',divt,plond,dummy)
      call outfld('DIVQ3D',divq3d,plond,dummy)
      call outfld('DIVT3D',divt3d,plond,dummy)
!!$      call outfld('DIVU',divu,plond,dummy)
!!$      call outfld('DIVV',divv,plond,dummy)
      call outfld('PRECOBS',precobs,plond,dummy )
      call outfld('LHFLXOBS',lhflxobs,plond,dummy )
      call outfld('SHFLXOBS',shflxobs,plond,dummy )
!!$      call outfld('Q1OBS',q1obs,plond,dummy )
!!$      call outfld('Q2OBS',q2obs,plond,dummy )
   endif
