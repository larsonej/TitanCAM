...
c     endif
     ...
     $                             rhoa3(ixyz)/rhoaold(ixyz)
        ...
        dx3(ixyz) = dx3(ixyz) * ( rhoaold(ixyz) / rhoa3(ixyz) ) ** 0.5
        dy3(ixyz) = dy3(ixyz) * ( rhoaold(ixyz) / rhoa3(ixyz) ) ** 0.5

c jas  dx and dy need to be reset if varstep takes bad step
c      and tries again....      

        if ( do_varstep ) then
         write(LUNOPRT,*) 'need to add reseting machinery for dx and dy'
         stop 1
        endif
      enddo
