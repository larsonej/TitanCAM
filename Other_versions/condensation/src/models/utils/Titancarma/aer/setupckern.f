...
c    <NR_DATA> is number of radius bins in input data
      ...
      parameter ( NP_DATA = 21, NR_DATA = 12 )
c
...
c
     ...
     $ 1.0000, 1.0000, 1.0000, 1.0000, 1.0000, 1.0000, 1.0000, 1.0000, 
     $ 1.0000, 1.0000, 1.0000, 1.0000, 1.0000 /
      data data_r(12), (data_e(ip,12),ip=1,NP_DATA) / 1100.0,
...
c  Define formats
c
    1 format(5i5,1pe13.6)
    2 format(5i5,2(1pe13.6))
    3 format(5i5,5(1pe15.4))
    4 format(7(1pe15.4))
    5 format(2i5,5(1pe15.4))
c
c
c-------------------------------------------------------------------------------
c
...
c  
      do jg = 2, NGROUP
        do ig = 1, jg-1
          if( icoag(ig,jg) .gt. ig ) then
            write(*,*) 'Error in <icoag)>: ',
     $                 'Must have icoag(ig,jg) < ig'
            stop
          else
...
c   use constant values (for smaller droplet, if available)
c   as in Beard and Ochs (1984)
c
...
c
c   Find <jp> such that data_p(jp) <= pr <= data_p(jp+1)                     
c
                jp = NP_DATA
                do jj = NP_DATA-1, 2, -1
                ...
                enddo
c
c   <pblni> is fractional distance of <pr> between points in <data_p> 
c
...
c
c    Find <jr> such that data_r(jr) <= r_larg <= data_r(jr+1)
c
                  jr = NR_DATA
                  do jj = NR_DATA-1, 1, -1
                  ...
                  enddo
c
c   <rblni> is fractional distance of <r_larg> between points in <data_r> 
c
...
c  overall kernel.
c
c           ckernel(k,i1,i2,j1,j2) = cbr + ccd + cgr
...
c           if(j1.ne.1)
c    $        write(LUNOTEMP,2) k,i1,i2,j1,j2,e_coal,e_coll
