...
c 
c
    2 format(f4.2,f8.2,1pe25.13)
    3 format(a,4(1pe13.6,2x))
    4 format(i4,3(1pe13.6,2x))
c
...
c
     ...
     $   ( vertdifu(k+1) + vertdifd(k) + vertadvu(k+1) + vertadvd(k) )
        if( cour .lt. 0.d0 .and. uc .ne. ONE )then
          uc = ONE
...
c
     ...
     $             + dz2(ixy,k)/dtime )
        ...
        dl(k) = cvert(k) *
     $     ( (ONE-uc)*(vertdifd(k)+vertdifu(k+1)+
     $                vertadvd(k)+vertadvu(k+1))
     $     - dz2(ixy,k)/dtime ) -
     $     (ONE-uc) * ( (vertdifu(k)+vertadvu(k))*ctempl(k) +
...
c
c  Mass conservation test (set fluxes into/out of model to zero)
c
       if( test_mass_cons ) then
         ftop = 0.d0
         fbot = 0.d0
       endif
