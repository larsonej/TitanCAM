      subroutine freezmixedl
c
c
c  @(#) freezmixedl.f  Jensen  Jan-2000
c  This routine evaluates particle loss rates due to nucleation <rnuclg>:
c  mixed hydrometeor freezing only.
c  
...
c
      if( DEBUG ) write(LUNOPRT,'(/,a)') 'Enter freezmixedl'
c
c
c  Loop over particle groups.
c
      do igroup = 1,NGROUP
c EJL 2-14-13 ad phased after mixed below
       if( is_grp_mixed_phase(igroup) ) then
...
c
         if( inucproc(iepart,ienucto) .eq. I_MIXEDFREEZE ) then
c
c
...
c
c
c  Set <rnuclg> to 1.e3 if T < 0C and the total core mass fraction = 1
c  (meaning the mixed particle is entirely frozen)
c
     ...
     $             ( pc3(ixyz,ibin,iepart) *rmass(ibin,igroup) )
             ...
             if( core_mass_frac .gt. 0.999 ) then
