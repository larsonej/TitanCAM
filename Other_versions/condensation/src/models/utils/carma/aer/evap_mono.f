...
c
c  Now the CN cores
c EJL 2-14-13 Added much of below from Titancarma
        do ic = 2, ncore(ig)
...
!EJL 3-27-13 Changeed if statement, added first '.eq. I_COREMASS'
             if( if_sec_mom(ig) .eq. I_COREMASS 
     $           .and. itype(iecore) .eq. I_COREMASS) then
               iemom = imomelem(ig)
...
c
c  Now the cores
c EJL 2-14-13 Added ifs below from Titancarma
c
       do ic = 2, ncore(ig)
         iecore = icorelem(ic,ig)
         ie2cn  = ievp2elem(iecore)
