...
c
c         if( itype(iefrom) .eq. I_INVOLATILE .or.
c     $       itype(iefrom) .eq. I_VOLATILE )then
c           ipow_from = 0
c         elseif ( itype(iefrom) .eq. I_COREMASS .or.
c     $            itype(iefrom) .eq. I_VOLCORE )then
c           ipow_from = 1
c         else
c           ipow_from = 2
c         endif
c
c         if( itype(ielem) .eq. I_INVOLATILE .or.
c     $       itype(ielem) .eq. I_VOLATILE )then
c           ipow_to = 0 
c         elseif ( itype(ielem) .eq. I_COREMASS .or.
c     $            itype(ielem) .eq. I_VOLCORE )then
c           ipow_to = 1 
c         else
c           ipow_to = 2 
c         endif
c
c         ipow = ipow_to - ipow_from
c
c
...
c
c EJL 3-23-13 added below. Not sure if rmass(ibin... below should
c be elemass?
