...
c
c
          thcond_h2o = 5.60e4 + (t3(ixyz)-T0(igas))*1.65e2  ! From CRC values
...
c EJL 2-12-13 changed 1 to igas in rlhm
           rmice_change = pc3(ixyz,ibin,ielem) - pc_premelt
           rlheat = rlheat + rmice_change * rlhm(iz,igas) /
     $                       ( CP * rhoa_cgs )

          endif
c EJL 2-13-13 changed below, switched top if and do
         endif     ! end of do ibin loop
        enddo    ! end of pconmax .gt. FEW_PC
