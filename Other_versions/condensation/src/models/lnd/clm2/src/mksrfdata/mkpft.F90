...
! !ARGUMENTS:
  ...
  real(r8), optional, intent(out) :: pctpft_max(lsmlon,lsmlat,maxpatch_pft) ! output grid PFT cover (% of vegetated area)
  integer , optional, intent(out) :: pft_max(lsmlon,lsmlat,maxpatch_pft)    ! output grid PFT (0 to numpft)
!
! !CALLED FROM:
...
!CSD$ &  wst, wst_sum, sumpct, wsti, diff)
  do jo = 1, lsmlat
     do io = 1, numlon(jo)
        ...
        if (present(pft_max) .and. present(pctpft_max)) then
           ...
           if (landmask(io,jo) == 1) then       !model grid wants land
              do m = 1, maxpatch_pft
                 if (wsti(m) .ne. miss) then
                    pft_max(io,jo,m) = wsti(m)
                    pctpft_max(io,jo,m) = wst(wsti(m))
                 else
                    pft_max(io,jo,m) = noveg
                    pctpft_max(io,jo,m) = 0.
                 end if
                 sumpct = sumpct + pctpft_max(io,jo,m)
              end do
           ...
           if (sumpct < wst_sum) then
              ...
              sumpct = 0.
              do m = 1, maxpatch_pft
                 pctpft_max(io,jo,m) = pctpft_max(io,jo,m) + diff/maxpatch_pft
                 sumpct = sumpct + pctpft_max(io,jo,m)
              end do
           end if
           ...
           do m = 1, maxpatch_pft
              if (pft_max(io,jo,m) < 0 .or. pft_max(io,jo,m) > numpft) then
                 write (6,*) 'MKPFT error: invalid PFT for i,j=',io,jo,pft_max(io,jo,m)
                 call endrun()
              end if
           end do
           ...
           if (landmask(io,jo) == 1) then
              if (abs(sumpct - 100.) > 0.000001) then
                 write(6,*)'MKPFT error: sum(pct) over maxpatch_pft is not = 100.'
                 write(6,*) sumpct, io,jo
                 call endrun()
              end if
              if (sumpct < -0.000001) then
                 write(6,*)'MKPFT error: sum(pct) over maxpatch_pft is < 0.'
                 write(6,*)sumpct, io,jo
                 call endrun()
              end if
           end if
