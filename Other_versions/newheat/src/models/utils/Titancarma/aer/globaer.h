...
!
!
!  Start of user-defined symbolic constants (all of which are array dimensions)
!
!
...
!
      ...
      parameter( NCCNBIN = 35 )
!
!
!  Define # particle elements 
!
!    parameter( NELEM = 2)  ! haze only model
      parameter( NELEM = 5)  ! methane cloud w/ volcore
!
!
...
!
!
!  Define logical unit number for radiation submodel print output
!
...
!
      ...
      parameter( I_VAPORDEP = 7 )
!
!
!  Define values of flag used to specify direction in
...
!  --now function of <igas>, set in {initgas}
!     parameter( T0 = 90.348d+0 )  !ethane
!     parameter( T0 = 90.69d+0  )  !methane
!     parameter( T0 = 80.6d+0   )  !CH4 + N2
!
!
...
!
!
!  Define gas constant for dry air [ erg / deg_K / g ]
!
...
!
!
!  Define mass density of liquid ethane [ g / cm^3 ]   !These densities should
!                                                      !really be defined for
      parameter( RHO_W = 0.5446d+0 ) !ethane           !each gas
!     parameter( RHO_W = 0.4228d+0 ) !methane
!
!
!  Define mass density of ethane ice [ g / cm^3 ]
!
      parameter( RHO_I = 0.713d+0 )  !ethane
!     parameter( RHO_I = 0.519d+0 )  !methane
!
!
!  Define Earth year [s]
! 
...
!   trhoifil  Name of input temperatures and densities file
!   tempofil  Name of temporary output file
!   fluxifil  Name of aerosol flux input file
!   apcifil1  Name of particle concentration input file (0 - 98 km)
!   apcifil2  Name of particle concentration input file (100 km)
!   timeofil  Name of timestep output file
!   ext       File name extension to designate run
...
!   do_write  .t. if write statement is to be executed (for debugging)
!   iwa       altitude for writing
!   iwb       bin for writing
!
      ...
      character*(50) trhoifil1
      character*(50) trhoifil2
...
!
      common /aer0/&
       ...
       khist, do_write, iwa, iwb
!
      common /aer0s/&
       prtofil, resifil, resofil, hisofil, stepofil, radofil,&
       trhoifil1, trhoifil2, tempofil, fluxifil, apcifil1, apcifil2,&
       timeofil, ext
!
!
!$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
!   icoagelem_cm  Coagulation element mapping array (core2mom)      {setupcoag}
...
!   if_nuc_lh   Conditional array for nucleation latent heat          {setupaer}
...
!
      common /aer2/&
        ...
        if_sec_mom(NGROUP), rlh_nuc(NELEM,NELEM),&
        ...
        is_grp_ice(NGROUP), is_grp_mixed_phase(NGROUP),&
        is_grp_mixed_comp(NGROUP), inuc2bin(NBIN,NGROUP,NGROUP),&
        ...
        icoagelem_cm(NELEM,NGROUP),tpstart,&
        iaer2
!
      common /aer2s/&
...
!   rprod       Particles added due to mass production
!   ppd         Growth and coagulation production term
!   pls         Growth and coagulation loss term
!   pcmflux     Mass flux for particles across bottom boundary of layer {vertical}
!   gcmflux     Mass flux for gases across bottom boundary of layer {vertical}
!   dmdte_gro   Element growth rate {growevapl}
!   dmdt_gro    Group growth rate {growevapl}
!   gprod_grow  Gas production term from growth
!   uc          Determines implicit/explicit calculations for transport {versol}
!
      ...
      common /aer5/&
        ...
        evaplg(NBIN,NGROUP), evappe(NBIN,NELEM),evcore(NELEM),&
        evdrop,coreavg,coresig,&
        gasprod(NGAS), rlheat,&
        ...
        cmf(NBIN,NGROUP), &
        totevap(NBIN,NGROUP),too_small,too_big,nuc_small, &
        ...
        gprod_grow(NGROUP,NGAS),&
        gprod_drop(NGROUP,NGAS),gprod_mono(NGROUP,NGAS),&
        gprod_poly(NGROUP,NGAS),uc,&
        iaer5
...
!   cbr_term0     numerator of brownian coag. kernel    {setupckern}
!   cbr_term1     from denominator of br. coag. kern.   {setupckern}
!   cbr_term2     mulitplied by sticking coeff. in cbr  {setupckern}
...
!
      common /aer6/&
        ...
        ilow(NGROUP,NBIN,NBIN*NBIN+2*NBIN), &
        jlow(NGROUP,NBIN,NBIN*NBIN+2*NBIN),&
        iup(NGROUP,NBIN,NBIN*NBIN+2*NBIN),&
        jup(NGROUP,NBIN,NBIN*NBIN+2*NBIN),&
        npairl(NGROUP,NBIN),&
        npairu(NGROUP,NBIN),&
...
!   tinit     Initial air temperature at layer mid-pt [deg_K]    
...
!
      common /aer9/&
        ...
        t(NX,NY,NZ), tinit(NXYZ), told(NXYZ), pold(NXYZ), rhoaold(NXYZ),&
        rmu(NZ), thcond(NZ), w(NX,NY,NZP1), zmetold(NXYZ),&
        u(NX,NY,NZ), v(NX,NY,NZ), t_surf(NX,NY),&
        dkz(NX,NY,NZP1), dkx(NX,NY,NZP1), dky(NX,NY,NZP1),tlast,&
        iaer9
!
!
...
!   vp_CH4_adjust Factor for adjusting CH4 liq vapor pressure w/ N2 {setupgrow}
...
!   gro3      Growth kernel (multiple gases) [UNITS?]             {setupgkern}
...
!   T0        Triple point temp for pure substance                {initgas}
...
!
      common /aer10/&
        ...
        ft(NZ,NBIN,NGROUP),& 
        gro(NZ,NBIN,NELEM), gro1(NZ,NBIN,NELEM),&
        gro2(NZ,NELEM), gro3(NZ,NELEM), &
...
!   iaer11   Safety marker for common block aer11 
!
      common /aer11/&
