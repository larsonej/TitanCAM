subroutine raddedmx(coszrs  ,ndayc   ,tau_rayleigh,pflx    ,ns      , &
...
!
   ...
   real(r8), intent(in) :: tau_rayleigh(pcols,0:pver)
   ...
   real(r8), intent(in) :: tauxar(pcols,0:pver) ! Aerosol extinction optical depth, should include carma
...
! FAO: input arguments intended to replace individual abs coef and abundances
   real(r8), intent(in) :: taugab(pcols,0:pver)   ! Layer total gas absorption optical depth
...
!
   real(r8), intent(inout) :: rdir(Nf,pcols,0:pver)   ! Layer reflectivity to direct rad
   real(r8), intent(inout) :: rdif(Nf,pcols,0:pver)   ! Layer reflectivity to diffuse rad
   real(r8), intent(inout) :: tdir(Nf,pcols,0:pver)   ! Layer transmission to direct rad
   real(r8), intent(inout) :: tdif(Nf,pcols,0:pver)   ! Layer transmission to diffuse rad
   real(r8), intent(inout) :: explay(Nf,pcols,0:pver) ! Solar beam exp transm for layer
!
! Corresponding quantities for clear-skies
!
   real(r8), intent(inout) :: rdirc(Nf,pcols,0:pver)  ! Clear layer reflec. to direct rad
   real(r8), intent(inout) :: rdifc(Nf,pcols,0:pver)  ! Clear layer reflec. to diffuse rad
   real(r8), intent(inout) :: tdirc(Nf,pcols,0:pver)  ! Clear layer trans. to direct rad
   real(r8), intent(inout) :: tdifc(Nf,pcols,0:pver)  ! Clear layer trans. to diffuse rad
   real(r8), intent(inout) :: explayc(Nf,pcols,0:pver)! Solar beam exp transm clear layer
!
!---------------------------Local variables-----------------------------
!
...
!
   ...
   real(r8) xgamma                ! Term in direct reflect and transmissivity
! EJL 7-7-13 changed gamma to xgamma due to intrinsic function named gamma
...
!
   ...
   xgamma(w,uu,g,e) = .50_r8*w*((3._r8*g*(1._r8-w)*uu*uu + 1._r8)/(1._r8-e*e*uu*uu))
...
!
   do k=0,pver
      do i=1,ndayc
            tautot = tauxcl(i,k) + tauxci(i,k) + tau_rayleigh(i,k) + taugab(i,k) + tauxar(i,k)
            taucsc = tauxcl(i,k)*wcl(i,k) + tauxci(i,k)*wci(i,k) + tauxar(i,k)*wa(i,k)
            wtau   = wray*tau_rayleigh(i,k)
            wt     = wtau + taucsc
            wtot   = wt/tautot
...
!            if (.not.masterproc .and. i.eq.1) then
!               write(6,'(a,i3,i3,i3,e12.4,e12.4,e12.4,e12.4,e12.4,e12.4)') 'FLUX1 ', &
!                    ns,k,i, &
!                    tau_rayleigh(i,k), &
!                    tauxcl(i,k), &
!                    taugab(i,k), &
!                    tauxar(i,k), &
!                    wtau*fray + fcl(i,k)*wcl(i,k)*tauxcl(i,k), &
!                    fci(i,k)*wci(i,k)*tauxci(i,k) + fa(i,k) *wa(i,k) *tauxar(i,k)
!            endif
...
!
            if (tauxcl(i,k) == 0.0_r8 .and. tauxci(i,k) == 0.0_r8) then
               ...
               tautot = tau_rayleigh(i,k) + taugab(i,k) + tauxar(i,k)
