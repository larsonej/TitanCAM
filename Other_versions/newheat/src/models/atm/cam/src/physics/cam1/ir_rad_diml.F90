...
! 
!
!  AJF, 3-20-12:  Altered to use source function defined at mid-layer points.
!
!=======================================================================
...
subroutine get_ir_bndmean(ns,nzm,nzi,cza,emis_ir,taui,gws, &
     planckf,planckfg,flwds,fnl, debug_rad)

!=======================================================================
!
!  Compute the radiative flux at interfaces of the model (Direct Integration method)
!  Source function linear-in-tau interpolation of *LAYER* Planck functions
!
!=======================================================================
!  USES:
...
! Arguments
   ...
   real(r8), intent(in) :: cza          ! cos-zenith-angle
...
! The following irradiances store averages over spectral interval [nu1,nu2]
   ...
   real(r8), parameter :: tau_max = 30.0
...
! Local variables:
   ...
   real(r8)  mu
   real(r8)  b0(pver),b1(pver),t1(pver),expt1(pver),df(pverp),uf(pverp),tx(pver)
   real(r8)  t1h(pver) !0.5*t1
   real(r8)  planckdiff(pver) ! Evaluated at interfaces 1 to pver
   real(r8)  fluxir(pverp)  !IR Flux at interfaces      
   

! ---------------------------------------------------------------------
   
    mu = cza  !Use same cza as shortwave calculation
  ...
  do k=1,ns
   ...
   t1 = taui(k,2:nzi)-taui(k,1:nzm) !Optical thickness of layer 1 to nzm
   t1h =0.5*t1
   tx = t1
   where (t1 > tau_max)
    tx = tau_max
...
! In mid-layer version, planckf(j+1) is the planck function of layer j, with 
!  planckf(1) corresponding to the atmosphere above the top interface
!  planckdiff(j) corresponds to dB centered on interface j

   planckdiff(1) =0.0  !Atm T above top interface = that of top layer
   planckdiff(2:nzm) = (planckf(k,3:nzi)-planckf(k,2:nzm)) !planckdiff(pver) = dB (pver intfc)  
                                                     

    b1(1) = 0.0
    b1(2:pver) = 2.*planckdiff(2:pver)/(t1(1:pver-1)+t1(2:pver))
    b0(1:pver) = planckf(k,2:nzi)   !b0(j) is planck function at layer j, = planckf(j+1)

     
! df = downward flux, uf = upward flux

   df(1) = 2.*pi*mu*b0(1)*(1.-exp(-taui(k,1)/mu))  ! Assumes b0(0)=b0(1)
   
   do j = 2,nzm
   i = j-1
   !print*,b0(i),b1(i)
   df(j) = df(i)*expt1(i) + 2.*pi*mu*(b0(i)*(1.-expt1(i)) + &
            b1(j)*(t1h(i)-mu +(t1h(i)+mu)*expt1(i) ))
   enddo
   j=nzi  !for downward flux at surface, use value of b1 at pver:
   i=j-1
   df(j) = df(i)*expt1(i) + 2.*pi*mu*(b0(i)*(1.-expt1(i)) + &
            b1(i)*(t1h(i)-mu +(t1h(i)+mu)*expt1(i) ))   
   


!!   uf(nzi) = 2.0*pi*mu*planckfg(k)*emis + df(nzi)*(1.0-emis)
!!!! In general, we want emis to depend on wavenumber.  But we
!!!! currently have a dimensioning problem, so a scalar emis_ir is
!!!! being passed thru from radclwmx_opgcm down to here:

#ifdef _TITAN
   uf(nzi) = 2.0*pi*mu*planckfg(k)*emis_ir + df(nzi)*(1.0-emis_ir)
#else
   uf(nzi) = 2.0*pi*mu*(planckf(k,nzi) + mu*b1(nzm)) !*May not be correct for outer planet
#endif

   !print*,'emis_ir, df(nzi), un(nzi) = ',emis_ir,df(nzi),uf(nzi)
   do j = 2,nzi
   
   n = nzi-j+1 !Runs from nzm to 1

    uf(n) = uf(n+1)*expt1(n) + 2.*pi*mu*(b0(n)*(1.-expt1(n)) - &       
        b1(n)*(t1h(n)-mu +(t1h(n)+mu)*expt1(n) ))         
