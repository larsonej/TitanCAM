...
!   print*, 'in CARMA ', eccf
    ...
    do i = 1, NELEM
      do j = 1, NBIN
        ...
        ! Only putting dust into one bin.
...
!                 (state%lon(icol) <= (35._r8*DEG2RAD)))) then
              ...
              do k = 1, 1 !! 1,pver when inputting in multiple layers
...
!                  rate = 1._r8
!EJL- gauss                   rate = 1e-14  
                  ! Calculate the mass flux in terms of kg/m3/s
!EJL - gauss                  massflux = (rate * rmass(j, igelem(i)) * 1.0e-3_r8 * 1.0e6_r8)
                  ! Convert the mass flux to a tendency on the mass mixing ratio.

                  !EJL - calculating a gaussian vertical emission profile.
                  carma_emis_expected =1.e-13!  * eccf             ! total emission [kg/m^2/s] - lav = 3.e-13
!                  sigma = 2.5e4         ! full width half max in (m)
!                  peak = 3.5e5        ! peak of guassian (m)
!EJL - gauss                  production(k)=(carma_emis_expected/(sigma*(2*SHR_CONST_PI)**.5))*EXP(-(state%zi(icol, k)-peak)**2/(2*sigma**2))
                  production(k)=carma_emis_expected
  
!EJL - gauss                  thickness = state%zi(icol, k) - state%zi(icol, k+1)
                  
!EJL- gauss                  ptend%q(icol, k, mm) = (massflux * thickness) / (state%pdel(icol, k) / gravit)       
...
!write(99,*) ''
!write(99,*) 'zmet_edge(NZP1), statezi(1,pver) - state%zi(1, pver+1)'
