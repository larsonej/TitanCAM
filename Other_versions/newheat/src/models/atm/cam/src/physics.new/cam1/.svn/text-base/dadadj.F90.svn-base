...
subroutine dadadj_trop (lchnk   ,ncol    , &
                   pmid    ,pint    ,pdel    ,t       , &
                   q       )
!----------------------------------------------------------------------- 
! 
...
! GFDL style dry adiabatic adjustment
!
!!!!!!  AJF 7/08/07:  Set this up for troposphere  !!!!!!
! 
! Method: 
! if stratification is unstable, adjustment to the dry adiabatic lapse
! rate is forced subject to the condition that enthalpy is conserved.
...
!-----------------------------------------------------------------------
   ...
   integer niter           ! number of iterations for convergence
   parameter (niter = 15)
   integer, parameter :: nlvdry_trop=48 !puts top of conv zone near 300mb for pver=61 
...
!-----------------------------------------------------------------------
!
   zeps = 2.0e-5           ! set convergence criteria - this looks ok for Titan
!
! Find gridpoints with unstable stratification
!
   k=pver  !start with deepest layer (ajf)

   do i=1,ncol
      gammad = cappa*0.5*(t(i,k) + t(i,k-1))/pint(i,k)
      dtdp = (t(i,k) - t(i,k-1))/(pmid(i,k) - pmid(i,k-1))
      dodad(i) = (dtdp + zeps) .gt. gammad
   end do
   do k=pver-1,nlvdry_trop,-1
      do i=1,ncol
         gammad = cappa*0.5*(t(i,k-1) + t(i,k))/pint(i,k)
         dtdp = (t(i,k) - t(i,k-1))/(pmid(i,k) - pmid(i,k-1))
         dodad(i) = dodad(i) .or. (dtdp + zeps).gt.gammad
      end do
   end do
!
! Make a dry adiabatic adjustment
! Note: nlvdry_trop ****MUST**** be > 1
!
   do 80 i=1,ncol
      if (dodad(i)) then
         zeps = 2.0e-5
         do k=pver,nlvdry_trop,-1
            c1dad(k) = cappa*0.5*(pmid(i,k)-pmid(i,k-1))/pint(i,k)
            c2dad(k) = (1. - c1dad(k))/(1. + c1dad(k))
            rdenom = 1./(pdel(i,k-1)*c2dad(k) + pdel(i,k))
            c3dad(k) = rdenom*pdel(i,k-1)
            c4dad(k) = rdenom*pdel(i,k)
         end do
50       do jiter=1,niter
            ilconv = .true.
            do k=pver,nlvdry_trop,-1
               zepsdp = zeps*(pmid(i,k) - pmid(i,k-1))
               zgamma = c1dad(k)*(t(i,k) + t(i,k-1))
               if ((t(i,k)-t(i,k-1)) >= (zgamma+zepsdp)) then
                  ilconv = .false.
                  t(i,k) = t(i,k-1)*c3dad(k) + t(i,k)*c4dad(k)
                  t(i,k-1) = c2dad(k)*t(i,k)
                  qave = (pdel(i,k-1)*q(i,k-1) + pdel(i,k)*q(i,k))/(pdel(i,k-1)+ pdel(i,k))
                  q(i,k) = qave
                  q(i,k-1) = qave
               end if
...
!
         zeps = zeps + zeps
         if (zeps > 1.e-4) then
            write(6,*)'DADADJ_TROP: No convergence in dry adiabatic adjustment'
            write(6,800) get_lat_p(lchnk,i),get_lon_p(lchnk,i),zeps
            call endrun
         else
            write(6,810) zeps,get_lat_p(lchnk,i),get_lon_p(lchnk,i)
