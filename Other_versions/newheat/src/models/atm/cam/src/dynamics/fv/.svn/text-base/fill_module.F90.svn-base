...
! Copy & swap direction for vectorization.
  ...
  do i=2,im-1
     do j=j1,j2
        if(qtmp(j,i) < 0.) then
           tinyl = max(0.,qtmp(j,i-1),qtmp(j,i+1))*tiny
           ipx =  1
! west
           d0 = max(0.,qtmp(j,i-1))
           d1 = min(-qtmp(j,i),d0)
...
! east
  ...
  do j=j1,j2
     if(qtmp(j,i) < 0.) then
        ipx =  1
        tinyl = max(0.,qtmp(j,im),qtmp(j,i+1))*tiny
! west
        d0 = max(0.,qtmp(j,im))
        d1 = min(-qtmp(j,i),d0)
...
! east
  ...
  do j=j1,j2
     if(qtmp(j,i) < 0.) then
        ipx =  1
        tinyl = max(0.,qtmp(j,i-1),qtmp(j,1))*tiny
! west
        d0 = max(0.,qtmp(j,i-1))
        d1 = min(-qtmp(j,i),d0)
        qtmp(j,i-1) = qtmp(j,i-1) - d1
...
! Take mass from west (essentially adjusting fx(i,j))
    ...
    do j=j1,j2
       do i=1,im
          if (qtmp(j,i) < -1.0e-10) then
             write (6,*) ' filew still failed ', qtmp(j,i), q(i,j)
          endif
          q(i,j) = qtmp(j,i)
       enddo
    enddo
