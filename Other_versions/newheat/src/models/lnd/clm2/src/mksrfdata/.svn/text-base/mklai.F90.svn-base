...
! !ARGUMENTS:
  ...
  integer , intent(in), optional :: pft_max(lsmlon,lsmlat,maxpatch_pft) ! PFT value(0 to numpft)
!
! !CALLED FROM:
...
!-----------------------------------------------------------------------
  ...
  if (ier /= 0) then
     write(6,*)'mklai allocation error'; call endrun()
  end if
  if (present(pft_max)) then
     allocate(mlai_max(lsmlon,lsmlat,maxpatch_pft), &
              msai_max(lsmlon,lsmlat,maxpatch_pft), &
              mhgtt_max(lsmlon,lsmlat,maxpatch_pft), &
              mhgtb_max(lsmlon,lsmlat,maxpatch_pft), stat=ier)
  end if
  if (ier /= 0) then
     write(6,*)'mklai allocation error'; call endrun()
  end if
     ...
     do jo = 1, lsmlat
        do io = 1, numlon(jo)
           ...
           if (present(pft_max)) then
              ...
              do l = 0, numpft
                 do n = 1, maxpatch_pft
                    if (l == pft_max(io,jo,n)) then
                       mlai_max(io,jo,n) = mlai_o(io,jo,l)
                       msai_max(io,jo,n) = msai_o(io,jo,l)
                       mhgtt_max(io,jo,n) = mhgtt_o(io,jo,l)
                       mhgtb_max(io,jo,n) = mhgtb_o(io,jo,l)
                    end if
                 end do
              end do
...
!$OMP END PARALLEL DO
     ...
     if (present(pft_max)) then
        ...
        call check_ret(nf_put_vara_double(ncido, varid, beg4d, len4d, mlai_max), subname)

        call check_ret(nf_inq_varid(ncido, 'MONTHLY_SAI', varid), subname)
        call check_ret(nf_put_vara_double(ncido, varid, beg4d, len4d, msai_max), subname)

        call check_ret(nf_inq_varid(ncido, 'MONTHLY_HEIGHT_TOP', varid), subname)
        call check_ret(nf_put_vara_double(ncido, varid, beg4d, len4d, mhgtt_max), subname)

        call check_ret(nf_inq_varid(ncido, 'MONTHLY_HEIGHT_BOT', varid), subname)
        call check_ret(nf_put_vara_double(ncido, varid, beg4d, len4d, mhgtb_max), subname)
...
1002    format (1x,i3,f16.3,f17.3)
  ...
  if (present(pft_max)) deallocate(mlai_max, msai_max, mhgtt_max, mhgtb_max)
