...
!------------------------------------------------------------------------
    ...
    if (flag == 'write') then
       ...
       if (masterproc) then
          ...
          ! FAO
          call check_ret( nf_def_dim(ncid, 'Nf_alb'  , 2, dimid), subname )
#ifdef RTM
          ...
          str = '$Id: inicFileMod.F90 46 2007-12-14 18:39:12Z hpc $'
       ...
       if (masterproc) then
          ...
          if (mss_irt > 0) then
             ...
             write(6,*) 'in lnd/inicfilemod', rem_fn
...
!------------------------------------------------------------------------
    ...
    if (flag == 'define') then
       ...
       ! FAO
       call ncd_defvar(ncid=ncid, varname='albedo_dir', xtype=nf_double,   &
            dim1name='column', dim2name='Nf_alb', long_name='albedo direct', units='unitless')
       call ncd_defvar(ncid=ncid, varname='albedo_dif', xtype=nf_double,   &
            dim1name='column', dim2name='Nf_alb', long_name='albedo diffuse', units='unitless')
...
!cdir nodep
       ...
       ! FAO
       call ncd_iolocal(varname='albedo_dir', data=clm3%g%l%c%cps%albgrd_static, datadim1='column', &
            datadim2='Nf_alb', ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) call endrun()

       call ncd_iolocal(varname='albedo_dif', data=clm3%g%l%c%cps%albgri_static, datadim1='column', &
            datadim2='Nf_alb', ncid=ncid, flag=flag, readvar=readvar)
       if (flag=='read' .and. .not. readvar) call endrun()
