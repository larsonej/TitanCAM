...
!-----------------------------------------------------------------------
   ...
   character(len=*), intent(out), optional :: carma_radofil_out
...
!-----------------------------------------------------------------------
   ...
   if ( present(carma_radofil_out) ) then
...
!-----------------------------------------------------------------------
   ...
   character(len=*), intent(in), optional :: carma_radofil_in
...
!-----------------------------------------------------------------------
   ...
   if ( present(carma_radofil_in) ) then
      carma_radofil = carma_radofil_in
   endif
...
!
	...
	real(r8) a                          ! total emission [kg/m^2/s]
	real(r8) sigma
    ...
    do i = 1, NELEM
      do j = 1, NBIN
		...
		if (cnst_name(mm) =="DUST40") then
          cflx(:ncol, mm) = -1e-13 ! EJL - surface constituent flux kg/m^2/s
          call outfld(trim(cnst_name(mm))//'SF', cflx(:, mm), pcols, lchnk)
        endif
		
        ! Only put dust into the smallest bin. 
...
!                  massflux = (rate * rmass(j, igelem(i)) * 1.0e-3_r8 * 1.0e6_r8)							
                  ...
                  !EJL - calculating a gaussian vertical emission profile. Needs to be fixed since vertical coordinate is not linear
				  a=1e-13             ! total emission [kg/m^2/s]
				  sigma = 5e4         ! full width half max in (m)
				  peak = 2.5e5        ! peak of guassian (m)
				  production(k)=(a/(sigma*(2*SHR_CONST_PI)**.5))*EXP(-(state%zi(icol, k)-peak)**2/(2*sigma**2))
  
                  thickness = state%zi(icol, k) - state%zi(icol, k+1)
				  
!                  ptend%q(icol, k, mm) = (massflux * thickness) / (state%pdel(icol, k) / gravit)       
				  ptend%q(icol,k,mm)=production(k)*thickness/(pressure/gravit)
				  !ptend%q(icol, 60, mm) = 1e-10     !EJL
!                end if
              enddo
!            end if
          enddo
