...
! !USES:
   ...
   use pmgrid, only: ompouter

#if defined( SPMD )
   use spmd_dyn, only: pkxy_to_pkc, ijk_yz_to_xy, geopk16byte
...
!-----------------------------------------
!EJL 8-20-13 I think I found the damping! tau1 0-8, with 0 at surf. and 8 at toa. 
!tau2 = 1-8 / 128*dt which is roughly 10^-4
!EJL 9-23-13 Recommendation from the telecon with Dr. Rood's group, set tau = 1
          do k=kfirst,klast
!!!            press = 0.5 * ( ak(k)+ak(k+1) + (bk(k)+bk(k+1))*1.E5 )
             press = 0.5 * ( ak(k)+ak(k+1) + (bk(k)+bk(k+1))*pstd )  !ajf, 3/7/07: correct pressure for titan
...
!EJL 9-23-13 recommended from Dr. Rood's group, remove the if statement and 
...
!$omp parallel do private(i, j, k, iord, jord)    
#endif
!EJL 9-23-13 recommendation from Dr. Rood's group, keep only last else branch of
!the if statements.
