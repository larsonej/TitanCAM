       subroutine coagp(ibinver,ielemver)
c
c
...
c
      do iquad = 1, npairu(igrp,ibinver)

       ig = igup(igrp,ibinver,iquad)           ! source group
       jg = jgup(igrp,ibinver,iquad)           ! source group
       i  = iup(igrp,ibinver,iquad)            ! source bin
       j  = jup(igrp,ibinver,iquad)            ! source bin

       iefrom = icoagelem(ielemver,ig)                ! source element for <i> particle
       if( if_sec_mom(igrp) )then
         iefrom_cm = icoagelem_cm(ielemver,ig)        ! core mass moment source element
       endif
c
...
c
       if( iefrom .ne. 0 ) then
        ...
        if( if_sec_mom(igrp) )then
          je_cm = icoagelem_cm(ielemver,jg)           ! core mass moment source element
        endif
c
c
...
c
         if( pconmax(ixyz,ig)*pconmax(ixyz,jg) .gt. FEW_PC**2 )then
          ...
          if( itype(ielemver) .eq. I_COREMASS .or.
     $        itype(ielemver) .eq. I_VOLCORE )then          ! core mass
...
c
          ...
          elseif( itype(ielemver) .eq. I_CORE2MOM )then      ! core mass^2
...
c 
          ...
          if( itype(ielemver) .ne. I_CORE2MOM )then
           coagpe(ixyz,ibinver,ielemver) = coagpe(ixyz,ibinver,ielemver)
...
c 
      do iquad = 1, npairl(igrp,ibinver)

       ig = iglow(igrp,ibinver,iquad)
       jg = jglow(igrp,ibinver,iquad)
       i  = ilow(igrp,ibinver,iquad)
       j  = jlow(igrp,ibinver,iquad)
