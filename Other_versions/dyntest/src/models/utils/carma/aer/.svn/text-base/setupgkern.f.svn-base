...
c
        ielem = ienconc(igroup)    ! particle number concentration element
...
c
     ...
     $                (xmet(ix,iy,k)*ymet(ix,iy,k)*zmet(ix,iy,k))
           ...
           if (igas .eq. 1 )then
c
c   Condensing gas is water vapor
c
c  <surfctwa> is surface tension of water-air interface (valid from 0 to 40 C)
c  from Pruppacher and Klett (eq. 5-12).
c
            surfctwa(k) = 76.10 - 0.155*( t(ix,iy,k) - 273.16 )
c
c  <surfctiw> is surface tension of water-ice interface
c  from Pruppacher and Klett (eq. 5-48).
c
            surfctiw(k) = 28.5 + 0.25*( t(ix,iy,k) - 273.16 )
c
c  <surfctiw> is surface tension of water-ice interface
c  from Hale and Plummer [J. Chem. Phys., 61, 1974].
c
            surfctia(k) = 141. - 0.15 * t(ix,iy,k)
c
c  <akelvin> is argument of exponential in kelvin curvature term.
c
            akelvin(k,igas) = 2.*gwtmol(igas)*surfctwa(k) 
     $                      / ( t(ix,iy,k)*RHO_W*RGAS )
 
            akelvini(k,igas) = 2.*gwtmol(igas)*surfctia(k) 
     $                      / ( t(ix,iy,k)*RHO_W*RGAS )
