...
#if (defined PERGRO)
       if (masterproc) then
          if (pertlim /= 0.0 .and. do_perturb) then
       ...
       do g = begg, endg
          ...
          if (prcxy(i,j) + prlxy(i,j) > 0.) then
             if (gptr%a2ls%forc_t(g) > (SHR_CONST_TKFRZ + tcrit)) then
                ...
                gptr%a2lf%forc_snow(g) = 0.
                gptr%a2ls%flfall(g) = 1.
             else
                gptr%a2lf%forc_rain(g) = 0.
                ...
                if (gptr%a2ls%forc_t(g) <= SHR_CONST_TKFRZ) then
                   gptr%a2ls%flfall = 0.0
                else if (gptr%a2ls%forc_t(g) <= SHR_CONST_TKFRZ+2.) then
                   gptr%a2ls%flfall(g) = coefa + coefb * gptr%a2ls%forc_t(g)
                else
                   gptr%a2ls%flfall(g) = coefa + coefb * (SHR_CONST_TKFRZ+2.)
                endif
#else
                if (gptr%a2ls%forc_t(g) <= SHR_CONST_TKFRZ) then
                   gptr%a2ls%flfall(g) = 0.
                else if (gptr%a2ls%forc_t(g) <= SHR_CONST_TKFRZ+2.) then
                   gptr%a2ls%flfall(g) = -54.632 + 0.2 * gptr%a2ls%forc_t(g)
                else
                   gptr%a2ls%flfall(g) = 0.4
                endif
#endif
             endif
          else
          ...
          if (pertlim /= 0.0 .and. do_perturb) then
             ...
             gptr%a2ls%forc_t(g) = (gptr%a2ls%forc_t(g))*(1. + pertval)
          endif
#endif
       end do
