...
! !USES:
    ...
    use clm_varcon         , only : cpair, vkc, grav, zvirl  !ajf,3/14/06
...
! this routine.
! 13 March 06: AJF: Noted ".0098" is actually g/Cp in "thm"
! 14  "    " : AJF: Changed "0.61" to "zvirl" = 1/epsi -1
! "   "    " : AJF: Changed air viscosity to be 2.1e-6 m^2/s at 95 K
!
! !LOCAL VARIABLES:
!
...
!
    ...
    real(r8), pointer :: thm(:)            ! intermediate variable (forc_t+g/Cp*forc_hgt_t)
...
!
    ...
    real(r8) :: visc                   !95-K viscosity of N2
!------------------------------------------------------------------------------
...
!cdir nodep
    do f = 1, fn
       ...
       dqh(p) = forc_q(g)-qg(c)
       dthv = dth(p)*(1.+zvirl*forc_q(g))+zvirl*forc_th(g)*dqh(p) !ajf,3/14/06
    ...
    do iter = 1, niters
       ...
       visc=2.1e-6 !m^2/s, N2 viscosity at 95 K, ajf 3/14/06
...
!cdir nodep
       do f = 1, fn
          ...
          tstar = temp1(p)*dth(p)
          qstar = temp2(p)*dqh(p)
          z0hg_pft(p) = z0mg_pft(p)/exp(0.13 * (ustar(p)*z0mg_pft(p)/visc)**0.45)
          z0qg_pft(p) = z0hg_pft(p)

          thvstar = tstar*(1.+zvirl*forc_q(g)) + zvirl*forc_th(g)*qstar !ajf,3/14/06
          ...
          if (zeta >= 0.) then                      !stable
             ...
             wc = beta(c)*(-grav*ustar(p)*thvstar*zii(c)/thv(c))**0.333
...
!EJL 8-19-13 - printing um
!    open(unit=99, file='bareground_Um.txt',status='unknown')
!    write(99,*) 'Um'
!    write(99,*) um(:)
!    write(99,*) 'Ustar'
!    write(99,*) ustar(:)
...
!cdir nodep
    do f = 1, fn
       ...
       ram     = 1./(ustar(p)*ustar(p)/um(p))
       rah     = 1./(temp1(p)*ustar(p))
       raw     = 1./(temp2(p)*ustar(p))
       ...
       !EJL 8-15-13 Added the *0.1 onto the taux and tauy
       taux(p)          = -forc_rho(g)*forc_u(g)/ram *0.01
       tauy(p)          = -forc_rho(g)*forc_v(g)/ram*0.01
       ...
       t_veg(p) = forc_t(g)
       btran(p) = 0._r8
       cf = forc_pbot(g)/(SHR_CONST_RGAS*0.001*thm(c))*1.e06
       rssun(p) = 1./1.e15 * cf
       rssha(p) = 1./1.e15 * cf
