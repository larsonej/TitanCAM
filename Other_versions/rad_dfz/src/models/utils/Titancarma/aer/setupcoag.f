...
c
    ...
    1 format('kbin(',i1,',1,1,',i2,',',i2,')=',i2,
     $       '   kbin(',i1,',1,2,',i2,',',i2,')=',i2,  
     $       '   kbin(',i1,',2,1,',i2,',',i2,')=',i2,  
     $       '   kbin(',i1,',2,2,',i2,',',i2,')=',i2  )
    2 format('setupcoag',10(i4))
c
c
...
c
     ...
     $          itype(ielem) .eq. I_VOLATILE ) )then !.or.
c    $          ncore(ig) .eq. 0 ) then
...
c
          else if( itype(ielem) .eq. I_COREMASS  .or.
...
c
c  If not using solutes in the model then check that compound names
c  match
c
          if( (itype(ielem) .eq. I_COREMASS .or.
     $         itype(ielem) .eq. I_GROWCORE )  .and.
     $        itype(ienconc(ig)).eq. I_INVOLATILE
     $        .and. ncore(ig) .eq. 0 ) then

            isolfrom = isolelem(ienconc(ig))

            if( isolfrom .eq. 0 ) then
              icompfrom = icomp(iepart)       ! source element compound
              if( icompfrom .ne. icompto )
     $          icoagelem(ielem,ig) = 0
            elseif( isolfrom .ne. isolto ) then
              icoagelem(ielem,ig) = 0
            endif
          endif
...
c               print 1, igrp,i,j,kbin(igrp,1,1,i,j)
c    $                  ,igrp,i,j,kbin(igrp,1,2,i,j)
c    $                  ,igrp,i,j,kbin(igrp,2,1,i,j)
c    $                  ,igrp,i,j,kbin(igrp,2,2,i,j)
...
c
          ...
          do jg = 1, NGROUP
            ...
            do j = 1, NBIN
              ...
              if( kb+1.eq.ibin .and. ncg.eq.igrp ) then
                ...
                maxnpair = max(maxnpair,npairu(igrp,ibin))
     ...
     $          (i.ne.ibin .or. ig.ne.igrp) ) then
                ...
                maxnpair = max(maxnpair,npairl(igrp,ibin))
...
c  Check that array bounds not being exceeded
      if( maxnpair .gt. NBIN*NBIN+2*NBIN ) then
         write(*,*) 'Need to increase array sizes (iup,jup, etc)'
         write(*,*) 'Max npair#',maxnpair
         stop
      endif
c
c
...
c      do ielem = 1, NELEM
c        do ig = 1, NGROUP
c          print*, 'ielem ig icoagelem_cm icomp(ielem) = ',
c    $     ielem, ig, icoagelem_cm(ielem,ig), icomp(ielem)
c        enddo
c      enddo
c      write(LUNOPRT,*) ' '
...
c  Leave out ckernel initially since it will be changed each timestep
