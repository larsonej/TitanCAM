...
c	 rf(NBIN,NGROUP,NVERT)		=  bin fractal radius [cm]
c    umon(NBIN,NGROUP,NVERT)		=  number of monomers per aggregate
c	 porosity(NBIN,NGROUP,NVERT)	=  porosity
c
      cpi = 4./3.*PI
...
c
!       if (j .eq. 12 ) then
!         open(unit=99,file='pre_NZ.txt',status='unknown')
!         write(99,*) r(j,igrp),umon(j,igrp),rmon(igrp)
!         write(99,*) 'igrp', igrp
!         close(unit=99)
!      endif
c
               do k = 1,NZ
c                 if (zl3(k) .gt. 30.d5 .and. zl3(k)  .lt. 80.d5) then 
                 if (k .gt. 28 .and. k .lt. 47) then
c                   df(j,igrp) = 2.6 - 1.0*exp(-umon(j,igrp)**(.5)/500.0)
                    df(j,igrp,k) = 2.4
                 elseif (k .ge. 47 ) then 
                   df(j,igrp,k) = 2.8
                 elseif (k .le. 28) then 
                 ...
                 rf(j,igrp,k) = r(j,igrp)**(3./df(j,igrp,k))* 
     $                  rmon(igrp)**(1.-3./df(j,igrp,k))
!       if (j .eq. 12) then
!       open(unit=99,file='df.txt',status='unknown')
!       write(99,*) rf(j,igrp,:)
!       write(99,*) rmon(igrp)
!       write(99,*) umon(j,igrp)
!       close(unit=99)
!       endif
c  
...
c  
     ...
     $           (3.*(1.-3./df(j,igrp,k)))
              porosity(j,igrp,k) = 1.0-(1.0 - tpor)*
     $                             sqrt(df(j,igrp,k)/3.0)
              gamm = (1. - porosity(j,igrp,k))**(1./3.)
              happel = 2./(9.*(1.-porosity(j,igrp,k)))* 
     $               (3.-4.5*gamm+4.5*gamm**5.0-3.*gamm**6.)/
     $               (3.+2.*gamm**5.)
              perm = happel*rmon(igrp)**2.0
              brinkman = umon(j,igrp)**(1./df(j,igrp,k))*
     $                   1./sqrt(happel)
              epsi = 1.0 - brinkman**(-1.)*tanh(brinkman)
              omega = 2.0/3.0*epsi/(2./3.+epsi/brinkman**2.0)
  
              rm(j,igrp,k) = rf(j,igrp,k) * omega
c  
              enddo
            endif
...
c
c printing test 
      open(unit=99, file='dfztest.txt',status='unknown')
      write(99,*) 'z  df  rf  rm porosity  r'
      do k=1,NZ
       write(99,'(i3,1pe11.3,1pe11.3,1pe11.3,1pe11.3,1pe11.3)') k, 
     $   df(15,1,k), rf(15,1,k), rm(15,1,k), porosity(15,1,k), r(15,1)
      enddo
      close(unit=99)
c  Print bin structure
     ...
     $          i,r(i,j),rf(i,j,15),rm(i,j,15),
     $          df(i,j,15),porosity(i,j,15),umon(i,j)
          enddo
        write(99,*) ''
      enddo
