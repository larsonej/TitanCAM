...
c
     ...
     $                  *GRAV*bpm(k,i,j)/rmu(k)/rm(i,j,k)
 
            re(k,i,j) = 2.*rhoa_cgs*rm(i,j,k)*vf(k,i,j)/rmu(k)
...
c
                cdrag = 0.45 
                vf(k,i,j) = sqrt( 8.*rhop2(ixy,k,i,j)*r(i,j)**3*GRAV*
     $                     bpm(k,i,j)/(3.*cdrag*rhoa_cgs*rm(i,j,k)**2) )
              endif
