...
C
 ...
 310  CONTINUE
C
C     WE DO NOT INCLUDE RAYLEIGH SCATTERING IN INFRARED
C
      DO 330 J          =   1,NVERT
         DO 320 L       =   1,NTOTAL
           if( L .LE. NSOLP ) then
...
C 
c
c     Limit x=2*pi/wave to no larger 1000 to avoid anguish in the Mie code.
c
...
C **********************************************************************
C
c     Set <iblackbody_above> = 1 to include a source of radiation
c     at the top of the radiative transfer model domain
c    
      iblackbody_above = ir_above_aerad
      t_above = tabove_aerad
c
c     Set <ibeyond_spectrum> = 1 to include blackbody radiation at
