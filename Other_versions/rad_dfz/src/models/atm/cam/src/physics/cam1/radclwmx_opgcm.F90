...
subroutine radclwmx_opgcm(lchnk,ncol, mmr_gas, pint, tnm, planckf_in, & 
...
!   real(r8), parameter :: emis_ir=PLANET_EMISSIVITY
...
#ifdef _CURTIS_MATRIX
      call get_cm_totflxs(Nf_lw,Ng_lw,num_g,weight_lw,taui_lw(lchnk,i,:,:,:), &
       czar,emis_ir,planckf(:,:,:,i),planckfg(:,:,i), &
       fnl_i,flwds(i),flns(i),flnsc(i),flnt(i),flut(i),flntc(i),flutc(i), &
       fcnl_i,debug_rad)
#else
...
#ifdef _CURTIS_MATRIX  !For C-M formulation, fnl(1:pver)=flux diff across layer
      do k=1,pver
         qrl(i,k) = shr_const_g*fnl(i,k)/(pint(i,k+1) - pint(i,k))
      end do
#else
      do k=1,pver
