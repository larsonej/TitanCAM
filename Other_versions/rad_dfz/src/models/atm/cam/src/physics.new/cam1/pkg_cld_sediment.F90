...
!---------------------------------------------------------------------------------
  ...
  private
  public :: cld_sediment_vel, cld_sediment_tend


  real (r8), parameter :: vland  = 1.5            ! liquid fall velocity over land  (cm/s)
  real (r8), parameter :: vocean = 2.8            ! liquid fall velocity over ocean (cm/s)
  real (r8), parameter :: mxsedfac   = 0.99       ! maximum sedimentation flux factor
...
! parameter for modified McFarquhar and Heymsfield
  real (r8), parameter :: vice_small = 1.         ! ice fall velocity for small concentration (cm/s)

! parameters for Stokes velocity
  real (r8), parameter :: eta =  1.7e-5           ! viscosity of air (kg m / s)
  real (r8), parameter :: r40 =  40.              !  40 micron radius
  real (r8), parameter :: r400= 400.              ! 400 micron radius
  real (r8), parameter :: v40 = (2./9.) * rhoh2o * gravit/eta * r40**2 * 1.e-12  
                                                  ! Stokes fall velocity of 40 micron sphere (m/s)
  real (r8), parameter :: v400= 1.00              ! fall velocity of 400 micron sphere (m/s)
...
! newway
             if (rel(i,k) < 40. ) then
                vfall = 2./9. * rhoh2o * gravit * rel(i,k)**2 / eta  * 1.e-12  ! micons^2 -> m^2
             else
...
! get effective radius
       ...
       do k = 1,pver
          do i = 1,ncol
             if (cloud(i,k) > 0. .and. cldice(i,k) > 0.) then
                if (rei(i,k) < 40. ) then
...
! calculated above at 2*icritc
             if (icice <= icritc) then
                ...
                vfall = vice_small * (1.-icefac) + vfall * icefac
             end if
...
! cloud overlap cloud factor
          cldovrl  = min( cloud(i,k) / (cldab(i)+.0001), 1._r8 )
          cldab(i) = cloud(i,k)
! evaporation into environment cause moistening and cooling
          evapliq = fxliq(i,k) * (1.-cldovrl) / (dtime * pdel(i,k))  ! into env (kg/kg/s)
          evapice = fxice(i,k) * (1.-cldovrl) / (dtime * pdel(i,k))  ! into env (kg/kg/s)
...
!     the minmod function 
    ...
    minmod(a,b) = 0.5*(sign(1._r8,a) + sign(1._r8,b))*min(abs(a),abs(b))
...
!        limit the interpolant
       ...
       if (abs(cfnew-cfint)/(abs(cfnew)+abs(cfint)+1.e-36)  .gt..03) then
...
!           now limit it
          ...
          ttt = minmod(qpl, qpr)
          tmin = min(0._r8,3*s(i,k),1.5*t,ttt)
          tmax = max(0._r8,3*s(i,k),1.5*t,ttt)
