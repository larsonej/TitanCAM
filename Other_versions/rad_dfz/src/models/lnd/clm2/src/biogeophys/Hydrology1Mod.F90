...
!cdir nodep
    do f = 1, num_nolakep
       ...
       if (itype(l)==istsoil .or. itype(l)==istwet) then
          ...
          if (frac_veg_nosno(p) == 1 .and. (forc_rain(g) + forc_snow(g)) > 0.) then
             ...
             fpi = 1. - exp(-0.5*(elai(p) + esai(p)))

             ! Direct throughfall
             qflx_through_snow(p) = forc_snow(g) * (1.-fpi)
             qflx_through_rain(p) = forc_rain(g) * (1.-fpi)
...
#if (defined OFFLINE)
          qflx_snow_grnd_pft(p) = qflx_prec_grnd(p)*(1.-flfall(g)) ! ice onto ground (mm/s)
...
!cdir nodep
    do f = 1, num_nolakec
       ...
       if (do_capsnow(c)) then
          ...
          if (forc_t(g) > tfrz + 2.) then
             bifall=50. + 1.7*(17.0)**1.5
          else if (forc_t(g) > tfrz - 15.) then
             bifall=50. + 1.7*(forc_t(g) - tfrz + 15.)**1.5
          else
             bifall=50.
          end if
       ...
       if (snl(c) == 0 .and. qflx_snow_grnd_col(c) > 0.0 .and. snowdp(c) >= 0.01) then
          newnode = 1
          snl(c) = -1
