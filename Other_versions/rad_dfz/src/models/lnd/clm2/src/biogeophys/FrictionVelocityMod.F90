...
!
   real(r8), parameter :: zetam = 1.574 ! transition point of flux-gradient relation (wind profile)
   real(r8), parameter :: zetat = 0.465 ! transition point of flux-gradient relation (temp. profile)
   ...
   do f = 1, fn
      ...
      if (zeta(p) < -zetam) then
         ustar(p) = vkc*um(p)/(log(-zetam*obu(p)/z0m(p))&
              ...
              + 1.14*((-zeta(p))**0.333-(zetam)**0.333))
      else if (zeta(p) < 0.) then
         ustar(p) = vkc*um(p)/(log(zldis(p)/z0m(p))&
              - StabilityFunc1(zeta(p))&
              + StabilityFunc1(z0m(p)/obu(p)))
      else if (zeta(p) <=  1.) then
         ustar(p) = vkc*um(p)/(log(zldis(p)/z0m(p)) + 5.*zeta(p) -5.*z0m(p)/obu(p))
      else
         ustar(p) = vkc*um(p)/(log(obu(p)/z0m(p))+5.-5.*z0m(p)/obu(p) &
              +(5.*log(zeta(p))+zeta(p)-1.))
      end if
      ...
      if (zeta(p) < -zetat) then
         temp1(p) = vkc/(log(-zetat*obu(p)/z0h(p))&
              ...
              + 0.8*((zetat)**(-0.333)-(-zeta(p))**(-0.333)))
      else if (zeta(p) < 0.) then
         temp1(p) = vkc/(log(zldis(p)/z0h(p)) &
              - StabilityFunc2(zeta(p)) &
              + StabilityFunc2(z0h(p)/obu(p)))
      else if (zeta(p) <=  1.) then
         temp1(p) = vkc/(log(zldis(p)/z0h(p)) + 5.*zeta(p) - 5.*z0h(p)/obu(p))
      else
         temp1(p) = vkc/(log(obu(p)/z0h(p)) + 5. - 5.*z0h(p)/obu(p) &
              + (5.*log(zeta(p))+zeta(p)-1.))
      end if
      ...
      if (forc_hgt_q(g) == forc_hgt_t(g) .and. z0q(p) == z0h(p)) then
         ...
         if (zeta(p) < -zetat) then
            ...
            temp2(p) = vkc/(log(zldis(p)/z0q(p)) + 5.*zeta(p)-5.*z0q(p)/obu(p))
         else
            temp2(p) = vkc/(log(obu(p)/z0q(p)) + 5. - 5.*z0q(p)/obu(p) &
                 + (5.*log(zeta(p))+zeta(p)-1.))
         end if
      endif
      ...
      if (zeta(p) < -zetat) then
         ...
         temp12m(p) = vkc/(log(zldis(p)/z0h(p)) + 5.*zeta(p) - 5.*z0h(p)/obu(p))
      else
         temp12m(p) = vkc/(log(obu(p)/z0h(p)) + 5. - 5.*z0h(p)/obu(p) &
              + (5.*log(zeta(p))+zeta(p)-1.))
      end if
      ...
      if (z0q(p) == z0h(p)) then
         ...
         if (zeta(p) < -zetat) then
            ...
            temp22m(p) = vkc/(log(zldis(p)/z0q(p)) + 5.*zeta(p)-5.*z0q(p)/obu(p))
         else
            temp22m(p) = vkc/(log(obu(p)/z0q(p)) + 5. - 5.*z0q(p)/obu(p) &
                 + (5.*log(zeta(p))+zeta(p)-1.))
         end if
      end if
      ...
      zldis(p) = forc_hgt_u(g)-displa(p)
      zeta(p) = zldis(p)/obu(p)
      if (min(zeta(p), 1.) < 0.) then
         tmp1 = (1. - 16.*min(zeta(p),1.))**0.25
         tmp2 = log((1.+tmp1*tmp1)/2.)
         tmp3 = log((1.+tmp1)/2.)
         fmnew = 2.*tmp3 + tmp2 - 2.*atan(tmp1) + 1.5707963
      else
         fmnew = -5.*min(zeta(p),1.)
      endif
      if (iter == 1) then
         fm(p) = fmnew
      else
      ...
      zeta10 = min(10./obu(p), 1.)
      if (zeta(p) == 0.) zeta10 = 0.
      if (zeta10 < 0.) then
...
!cdir nodep
   do f = 1, fn
      ...
      if (zeta(p) < -zetam) then           ! zeta < -1
         ustar(p) = vkc * um(p) / log(-zetam*obu(p)/z0m(p))
      else if (zeta(p) < 0.) then         ! -1 <= zeta < 0
         ustar(p) = vkc * um(p) / log(zldis(p)/z0m(p))
      else if (zeta(p) <= 1.) then        !  0 <= ztea <= 1
         ustar(p)=vkc * um(p)/log(zldis(p)/z0m(p))
...
! 15 September 1999: Yongjiu Dai; Initial code
! 15 December 1999:  Paul Houser and Jon Radakovich; F90 Revision
! 14 March 2006:  AJF; reduce initial wc from 0.5 to 0.1 m/s
!
!EOP
!
! !LOCAL VARIABLES:
...
!-----------------------------------------------------------------------
    ...
    ustar=0.06
    wc=0.1  !ajf 3-14-06, reduced from .5 to 0.1 m/s 
    if (dthv >= 0.) then
       um=max(ur,0.1_r8)
    else
       um=sqrt(ur*ur+wc*wc)
