...
!
  private
  real(r8), pointer :: rtmin_ave(:,:)        ! RTM averaging buffer for runoff, prec, evap
...
!-----------------------------------------------------------------------
    ...
    allocate(rtmin_ave_glob_sn(3,numg), rtmin_ave_glob_dc(3,numg), stat=ier)
    if (ier /= 0) then
    ...
    if (flag == 'read') then
       if (masterproc) then
          ...
          if (rest_id == 7) then
             read(nio, iostat=ier) runoff%lnd
          else if (rest_id == 6) then
             write(6,*)'RESTART_RTM id is 6'
             write(6,*)'RESTART_RTM: land runoff vector is not on restart file - values will be set to 0'
             write(6,*)'RESTART_RTM: ocean runoff vector is on restart file - must be rescaled by area*1000.'
             call shr_sys_flush(6)
             do n = 1,size(runoff%ocn)
                runoff%ocn(n) = runoff%ocn(n) * (runoff%ocn_area(n)*1000.)
             end do
             runoff%lnd(:) = 0.
          else
             write(6,*)'RESTART_RTM error: only restart_id 6 is supported for backwards compatibility'
             call shr_sys_flush(6)
             call endrun()
          end if
       ...
       call scatter_data_from_master (rtmin_ave, rtmin_ave_glob_dc, clmlevel=nameg)
#else
    ...
    if (flag == 'write') then
#if (defined SPMD)
       call gather_data_to_master (rtmin_ave, rtmin_ave_glob_dc, clmlevel=nameg)
#else
