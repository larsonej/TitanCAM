...
!cdir nodep
  do c = lbc, ubc
     dz(c,-nlevsno+1: 0) = 1.e36
     z (c,-nlevsno+1: 0) = 1.e36
     zi(c,-nlevsno  :-1) = 1.e36
  end do
...
!cdir nodep
  do c = lbc,ubc
     ...
     if (.not. lakpoi(l)) then
        if (snowdp(c) < 0.01) then
           snl(c) = 0
           dz(c,-nlevsno+1:0) = 0.
           z (c,-nlevsno+1:0) = 0.
           zi(c,-nlevsno+0:0) = 0.
        else
           if ((snowdp(c) >= 0.01) .and. (snowdp(c) <= 0.03)) then
              snl(c) = -1
              dz(c,0)  = snowdp(c)
           else if ((snowdp(c) > 0.03) .and. (snowdp(c) <= 0.04)) then
              snl(c) = -2
              dz(c,-1) = snowdp(c)/2.
              dz(c, 0) = dz(c,-1)
           else if ((snowdp(c) > 0.04) .and. (snowdp(c) <= 0.07)) then
              snl(c) = -2
              dz(c,-1) = 0.02
              dz(c, 0) = snowdp(c) - dz(c,-1)
           else if ((snowdp(c) > 0.07) .and. (snowdp(c) <= 0.12)) then
              snl(c) = -3
              dz(c,-2) = 0.02
              dz(c,-1) = (snowdp(c) - 0.02)/2.
              dz(c, 0) = dz(c,-1)
           else if ((snowdp(c) > 0.12) .and. (snowdp(c) <= 0.18)) then
              snl(c) = -3
              dz(c,-2) = 0.02
              dz(c,-1) = 0.05
              dz(c, 0) = snowdp(c) - dz(c,-2) - dz(c,-1)
           else if ((snowdp(c) > 0.18) .and. (snowdp(c) <= 0.29)) then
              snl(c) = -4
              dz(c,-3) = 0.02
              dz(c,-2) = 0.05
              dz(c,-1) = (snowdp(c) - dz(c,-3) - dz(c,-2))/2.
              dz(c, 0) = dz(c,-1)
           else if ((snowdp(c) > 0.29) .and. (snowdp(c) <= 0.41)) then
              snl(c) = -4
              dz(c,-3) = 0.02
              dz(c,-2) = 0.05
              dz(c,-1) = 0.11
              dz(c, 0) = snowdp(c) - dz(c,-3) - dz(c,-2) - dz(c,-1)
           else if ((snowdp(c) > 0.41) .and. (snowdp(c) <= 0.64)) then
              snl(c) = -5
              dz(c,-4) = 0.02
              dz(c,-3) = 0.05
              dz(c,-2) = 0.11
              dz(c,-1) = (snowdp(c) - dz(c,-4) - dz(c,-3) - dz(c,-2))/2.
              dz(c, 0) = dz(c,-1)
           else if (snowdp(c) > 0.64) then
              snl(c) = -5
              dz(c,-4) = 0.02
              dz(c,-3) = 0.05
              dz(c,-2) = 0.11
