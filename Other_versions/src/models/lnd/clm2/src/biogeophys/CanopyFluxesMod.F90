...
!
   real(r8), parameter :: btran0 = 1.0e-10  ! initial value
   real(r8), parameter :: zii = 1000.0      ! convective boundary layer height [m]
   real(r8), parameter :: beta = 1.0        ! coefficient of conective velocity [-]
   real(r8), parameter :: delmax = 1.0      ! maxchange in  leaf temperature [K]
   ...
   real(r8), parameter :: po2 = 0.209       ! partial pressure  o2 (mol/mol)
   real(r8), parameter :: pco2 = 355.e-06   ! partial pressure co2 (mol/mol)
   ...
   real(r8) :: co2(lbp:ubp)          ! atmospheric co2 concentration (pa)
   real(r8) :: o2(lbp:ubp)           ! atmospheric o2 concentration (pa)
   ...
   real(r8) :: rresis                ! soil water contribution to root resistance
...
!cdir nodep
      do f = 1, fn
         ...
         if (t_soisno(c,j) > tfrz) then
            ...
            rresis = (1.-smp_node/smpmax(ivt(p)))/(1.+sucsat(c,j)/smpmax(ivt(p)))
...
!cdir nodep
   do f = 1, fn
      ...
      air(p) =   emv(p) * (1.+(1.-emv(p))*(1.-emg(c))) * forc_lwrad(g)
      bir(p) = - (2.-emv(p)*(1.-emg(c))) * emv(p) * sb
      ...
      co2(p) = pco2 * forc_pbot(g)
      o2(p)  = po2 * forc_pbot(g)

      ! Initialize flux profile

      nmozsgn(p) = 0

      taf(p) = (t_grnd(c) + thm(c))/2.
      qaf(p) = (forc_q(g)+qg(c))/2.
   ...
   ITERATION : do while (itlef <= itmax .and. fn > 0)
      ...
      call FrictionVelocity (lbp, ubp, fn, filterp, &
           displa(lbp:ubp), z0mv(lbp:ubp), z0hv(lbp:ubp), z0qv(lbp:ubp), &
           obu(lbp:ubp), itlef+1, ur(lbp:ubp), um(lbp:ubp), ustar(lbp:ubp), &
           temp1(lbp:ubp), temp2(lbp:ubp), temp12m(lbp:ubp), temp22m(lbp:ubp), fm(lbp:ubp))
...
!cdir nodep
      do f = 1, fn
         ...
         ram1(p)  = 1./(ustar(p)*ustar(p)/um(p))
         rah(p,1) = 1./(temp1(p)*ustar(p))
         raw(p,1) = 1./(temp2(p)*ustar(p))

         ! Bulk boundary layer resistance of leaves

         uaf = um(p)*sqrt( 1./(ram1(p)*um(p)) )
         cf  = 0.01/(sqrt(uaf)*sqrt(dleaf(ivt(p))))
         rb(p)  = 1./(cf*uaf)
         ...
         w = exp(-(elai(p)+esai(p)))
         csoilcn = (vkc/(0.13*(z0mg(c)*uaf/1.5e-5)**0.45))*w + csoilc*(1.-w)
         rah(p,2) = 1./(csoilcn*uaf)
         raw(p,2) = rah(p,2)
         ...
         svpts(p) = el(p)                         ! pa
         eah(p) = forc_pbot(g) * qaf(p) / 0.622   ! pa
      end do

      call Stomata (fn, filterp, lbp, ubp, lbc, ubc, lbg, ubg, &
           parsun(lbp:ubp), svpts(lbp:ubp), eah(lbp:ubp), thm(lbc:ubc), forc_pbot(lbg:ubg),  &
           t_veg(lbp:ubp), o2(lbp:ubp), co2(lbp:ubp), btran(lbp:ubp), rb(lbp:ubp), &
           rssun(lbp:ubp), psnsun(lbp:ubp), annpsnpot(lbp:ubp), annpsn(lbp:ubp))

      call Stomata (fn, filterp, lbp, ubp, lbc, ubc, lbg, ubg, &
           parsha(lbp:ubp), svpts(lbp:ubp), eah(lbp:ubp), thm(lbc:ubc), forc_pbot(lbg:ubg), &
           t_veg(lbp:ubp), o2(lbp:ubp), co2(lbp:ubp), btran(lbp:ubp), rb(lbp:ubp), &
           rssha(lbp:ubp), psnsha(lbp:ubp), annpsnpot(lbp:ubp), annpsn(lbp:ubp))
...
!cdir nodep
      do f = 1, fn
         ...
         wtg(p) = 1./rah(p,2)                ! ground
         wtshi  = 1./(wta+wtl+wtg(p))
         ...
         dt_veg(p) = (sabv(p) + air(p) + bir(p)*t_veg(p)**4 + &
              ...
              (- 4.*bir(p)*t_veg(p)**3 +dc1*wtga +dc2*wtgaq*qsatldT(p))
         ...
         thvstar = tstar*(1.+0.61*forc_q(g)) + 0.61*forc_th(g)*qstar
         ...
         if (zeta >= 0.) then     !stable
            ...
            wc = beta*(-grav*ustar(p)*thvstar*zii/thv(c))**0.333
            um(p) = sqrt(ur(p)*ur(p)+wc*wc)
         end if
         obu(p) = zldis(p)/zeta

         if (obuold(p)*obu(p) < 0.) nmozsgn(p) = nmozsgn(p)+1
         if (nmozsgn(p) >= 4) obu(p) = zldis(p)/(-0.01)
         obuold(p) = obu(p)
...
!cdir nodep
   do f = 1, fn
      ...
      err(p) = sabv(p) + air(p) + bir(p)*tlbef(p)**3*(tlbef(p) + 4.*dt_veg(p)) &
      ...
      t_ref2m(p) = thm(c) + temp1(p)*dth(p)*(1./temp12m(p) - 1./temp1(p))

      ! 2 m height specific humidity

      q_ref2m(p) = forc_q(g) + temp2(p)*dqh(p)*(1./temp22m(p) - 1./temp2(p))

      ! Downward longwave radiation below the canopy

      dlrad(p) = (1.-emv(p))*emg(c)*forc_lwrad(g) + &
         emv(p)*emg(c)*sb*tlbef(p)**3*(tlbef(p) + 4.*dt_veg(p))

      ! Upward longwave radiation above the canopy

      ulrad(p) = ((1.-emg(c))*(1.-emv(p))*(1.-emv(p))*forc_lwrad(g) &
         + emv(p)*(1.+(1.-emg(c))*(1.-emv(p)))*sb*tlbef(p)**3*(tlbef(p) + &
         4.*dt_veg(p)) + emg(c)*(1.-emv(p))*sb*t_grnd(c)**4)
...
! !ARGUMENTS:
     ...
     integer , intent(in)    :: lbc, ubc           ! column bounds
     integer , intent(in)    :: lbg, ubg           ! gridcell bounds
     real(r8), intent(in)    :: apar(lbp:ubp)      ! par absorbed per unit lai (w/m**2)
     ...
     real(r8), intent(in)    :: tgcm(lbc:ubc)      ! air temperature at agcm reference height (kelvin)
     ...
     real(r8), intent(in)    :: btran(lbp:ubp)     ! soil water transpiration factor (0 to 1)
     real(r8), intent(inout) :: rb(lbp:ubp)        ! boundary layer resistance (s/m)
     real(r8), intent(inout) :: rs(lbp:ubp)        ! leaf stomatal resistance (s/m)
     real(r8), intent(inout) :: psn(lbp:ubp)       ! foliage photosynthesis (umol co2 /m**2/ s) [always +]
     real(r8), intent(inout) :: annpsnpot(lbp:ubp) ! annual potential photosynthesis (umol CO2 /m**2)
     real(r8), intent(inout) :: annpsn(lbp:ubp)    ! annual photosynthesis (umol CO2 /m**2)
!
! !CALLED FROM:
...
! 15 September 1999: Yongjiu Dai; Initial code
! 15 December 1999:  Paul Houser and Jon Radakovich; F90 Revision
! 1/30/02, PET  Made all input and output parameters explicit (removed reference to clm)
!
! !LOCAL VARIABLES:
!
! local pointers to implicit in variables
...
! !OTHER LOCAL VARIABLES:
!
     real(r8), parameter :: mpe = 1.e-6   ! prevents overflow error if division by zero
...
!cdir nodep
     do f = 1, fn
        ...
        rsmax0 = 2.e4
        cf = forc_pbot(g)/(SHR_CONST_RGAS*0.001*tgcm(c))*1.e06

        if (apar(p) <= 0.) then          ! night time
           rs(p) = min(rsmax0, 1./bp * cf)
           psn(p) = 0._r8
        else                             ! day time
           ...
           ko = ko25 * f1(ako,tc)
           awc = kc * (1.+o2(p)/ko)
           cp = 0.5*kc/ko*o2(p)*0.21
           vcmx = vcmx25(ivt(p)) * f1(avcmx,tc) / f2(tc) * btran(p)
#if (defined DGVM)
           ...
           ci = 0.7*co2(p)*c3psn(ivt(p)) + 0.4*co2(p)*(1.-c3psn(ivt(p)))
           ...
           cea = max(0.25*ei(p)*c3psn(ivt(p))+0.40*ei(p)*(1.-c3psn(ivt(p))), min(ea(p),ei(p)) )

#if (defined DGVM)
           ! ci iteration for 'potential' photosynthesis

           do iter = 1, niter
              wj = max(ci-cp,0._r8)*j/(ci+2.*cp)*c3psn(ivt(p)) + j*(1.-c3psn(ivt(p)))
              wc = max(ci-cp,0._r8)*vcmxpot/(ci+awc)*c3psn(ivt(p)) + vcmxpot*(1.-c3psn(ivt(p)))
              we = 0.5*vcmxpot*c3psn(ivt(p)) + 4000.*vcmxpot*ci/forc_pbot(g)*(1.-c3psn(ivt(p)))
              psn(p) = min(wj,wc,we)
              cs = max( co2(p) - 1.37*rb(p)*forc_pbot(g)*psn(p), mpe )
              atmp = mp(ivt(p))*psn(p)*forc_pbot(g)*cea / (cs*ei(p)) + bp
              btmp = ( mp(ivt(p))*psn(p)*forc_pbot(g)/cs + bp ) * rb(p) - 1.
              ctmp = -rb(p)
              if (btmp >= 0.) then
              ...
              r2 = ctmp/q
              rs(p) = max(r1,r2)
              ci = max( cs-psn(p)*forc_pbot(g)*1.65*rs(p), 0._r8 )
           end do
...
!NEC NEC NEC
           if ( niter /= 3 ) then
              write(6,*) 'Stomata: niter /= 3';  call endrun()
           end if
           !
           ! ITER = 1
           !
           wj = max(ci-cp,0._r8)*j/(ci+2.*cp)*c3psn(ivt(p)) + j*(1.-c3psn(ivt(p)))
           wc = max(ci-cp,0._r8)*vcmx/(ci+awc)*c3psn(ivt(p)) + vcmx*(1.-c3psn(ivt(p)))
           we = 0.5*vcmx*c3psn(ivt(p)) + 4000.*vcmx*ci/forc_pbot(g)*(1.-c3psn(ivt(p)))
           psn(p) = min(wj,wc,we)
           cs = max( co2(p)-1.37*rb(p)*forc_pbot(g)*psn(p), mpe )
           atmp = mp(ivt(p))*psn(p)*forc_pbot(g)*cea / (cs*ei(p)) + bp
           btmp = ( mp(ivt(p))*psn(p)*forc_pbot(g)/cs + bp ) * rb(p) - 1.
