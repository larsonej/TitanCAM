...
itype = lonarr(nelem)  &  igelem = itype
nelemg = lonarr(ngroups)  &  ncore = nelemg  &  ienconc = nelemg
readu,1, itype, igelem, nelemg, ncore, ienconc
print,'itype = ', itype
print,'igelem = ',igelem
print,'nelemg = ',nelemg
print,'ncore = ',ncore
print,'ienconc = ', ienconc
nxyz = nx*ny*nz
...
yline = [0.7,0.7]  &  oplot,xline,yline  &  xyouts, xline(1)+0.1,yline(0)*0.93, 'Numerical: Mrat='+ $
                                                    string(rmass(1,0)/rmass(0,0),form='(F5.2)'), size=1.1
