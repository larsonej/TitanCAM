...
!-----------------------------------------------------------------------
!
! $Id: scandyn.F90 62 2008-04-23 22:59:18Z cam_titan $
! $Author: cam_titan $
!
!-----------------------------------------------------------------------
...
! FFT buffers
!
   real(r8), allocatable:: fftbuf_in(:,:,:,:)          ! fftbuf_in(nlon_fft_in,plev,9,beglat:endlat) 
   real(r8), allocatable:: fftbuf_out(:,:,:,:)         ! fftbuf_out(nlon_fft_out,plev,9,plat)
!
   nlon_fft_in = plond
   allocate(fftbuf_in(nlon_fft_in,plev,9,beglat:endlat))
   ...
   nlon_fft_out = 2*maxm
#endif
   allocate(fftbuf_out(nlon_fft_out,plev,9,plat))
#else
   nlon_fft_out = 1
...
!$OMP PARALLEL DO PRIVATE (J, LAT)
   do lat=beglat,endlat
      ...
      call linemsdyn_bft (lat, nlon(lat), nlon_fft_in, &
                      ps(1,lat,n3m1), ps(1,lat,n3m2), u3(i1,1,j,n3m1), &
