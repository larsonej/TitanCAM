...
!-----------------------------------------------------------------------
   real(r8), private, parameter :: tmax_fice = tmelt - 10.       ! max temperature for cloud ice formation
...
!!$   real(r8), private, parameter :: tmin_fice = tmax_fice - 20.! min temperature for cloud ice formation
   real(r8), private, parameter :: tmin_fice = tmax_fice - 30.   ! min temperature for cloud ice formation
!  pjr
   real(r8), private, parameter :: tmax_fsnow = tmelt            ! max temperature for transition to convective snow
   real(r8), private, parameter :: tmin_fsnow = tmelt-5.         ! min temperature for transition to convective snow
...
!
   ...
   if ( dycore_is ('LR') ) then
      icritw = 2.e-4
      icritc = 9.5e-6    ! threshold for autoconversion of cold ice
      conke  =  5.e-6    ! tunable constant for evaporation of precip
   else
      if( get_resolution() == 'T85' ) then
         icritw = 4.e-4
         icritc = 16.0e-6    ! threshold for autoconversion of cold ice
         conke  = 5.e-6    ! tunable constant for evaporation of precip
      elseif( get_resolution() == 'T31' ) then
         icritw = 4.e-4
         icritc =  3.e-6    ! threshold for autoconversion of cold ice
         conke  = 10.e-6    ! tunable constant for evaporation of precip
      else
         icritw = 4.e-4
         icritc =  5.e-6    ! threshold for autoconversion of cold ice
         conke  = 10.e-6    ! tunable constant for evaporation of precip
      endif
   endif
...
!
   ...
   real(r8),parameter ::omsm=0.99999                 ! a number just less than unity (for rounding)
...
!
         evapprec(i,k) = conke*(1. - cldm(i))*sqrt(precab(i)) &
...
! rain produced in this layer not too effective in collection process
            wtthick = max(0._r8,min(0.5_r8,((zi(i,k)-zi(i,k+1))/1000.)**2))
...
!           rain accreting ice (and freezing), or by an explicit freezing of raindrops
            ...
            nliq2pr = min(liq2pr(i,k),(cwat(i,k)+cme(i,k)*deltat)*(1.-fice(i,k))/deltat)
!            write (6,*) ' prodprec ', i, k, prodprec(i,k)
!            write (6,*) ' nliq2pr, nice2pr ', nliq2pr, nice2pr
            if (liq2pr(i,k).ne.0.) then
...
!           final estimates of conversion to precip and snow
            ...
            rliq(i,l,k) = (cwat(i,k) + cme(i,k)*deltat)*(1.-fice(i,k)) - nliq2pr * deltat
...
!
      do i = 1,ncol
#ifdef PERGRO
         evapprec(i,k) = conke*(1. - max(cldm(i),mincld))* &
...
! begin sanity checks
   ...
   do k = k1mb,pver
      do l = 1,iter
         do i = 1,ncol
            if (rcwn(i,l,k).lt.0.) error_found = .true.
            if (rliq(i,l,k).lt.0.) error_found = .true.
            if (rice(i,l,k).lt.0.) error_found = .true.
         enddo
      enddo
   enddo
   if (error_found) then
      do k = k1mb,pver
         do l = 1,iter
            do i = 1,ncol
               ...
               if (rliq(i,l,k).lt.0.) then
                  write (6,*) ' prob with neg rliq1 ', rliq(i,l,k)
                  call endrun('PCOND')
               endif
               if (rice(i,l,k).lt.0.) then
...
!  capnsi = 40.              ! sea ice cloud particles density  / cm3
   capnsi = 75.              ! sea ice cloud particles density  / cm3
...
!   effc = 0.    ! turn off warm-cloud autoconv
   ...
   r3lcrit = 10.0e-6         ! 10.0u  crit radius where liq conversion begins
!
...
! jrm Reworked droplet number concentration algorithm
      ...
      capn = capnw + (capnc-capnw) * min(1._r8,max(0._r8,1.0-(p(i,k)-0.8*p(i,pver))/(0.2*p(i,pver))))
      ! Modify for snow depth over land
      capn = capn + (capnc-capn) * min(1.0_r8,max(0.0_r8,snowh(i)*10.))
      ! Ramp between polluted value over land to clean value over ocean.
      capn = capn + (capnc-capn) * min(1.0_r8,max(0.0_r8,1.0-landm(i,lchnk)))
