...
!-----------------------------------------------------------------------
!
! $Id: somini.F90 17 2006-12-11 21:50:24Z hpc $
! $Author: hpc $
!
...
!
      ...
      if (sstcyc) then
         if (mod (date_som(1), 10000)/100 /= 1) then
            call endrun ('SOMINI: 1st month must be 1 when cycling som.')
         end if
         if (mod(date_som(psomtim),10000)/100 /= 12) then  
            call endrun ('SOMINI: must have 12 consecutive months of data when cycling som.')
         end if
      end if
